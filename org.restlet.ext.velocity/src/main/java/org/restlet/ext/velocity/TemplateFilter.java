...
/**
 * Filter the response's entity and wrap it with a Velocity's template representation. By default,
 * the template representation provides a data model based on the request and response objects. In
 * order for the wrapping to happen, the representations must have the {@link Encoding#VELOCITY}
 * encoding set.<br>
 * <br>
...
public class TemplateFilter extends Filter {
    ...
    protected void afterHandle(Request request, Response response) {
        if (response.isEntityAvailable()
                && response.getEntity().getEncodings().contains(Encoding.VELOCITY)) {
            try {
                final TemplateRepresentation representation =
                        new TemplateRepresentation(
                                response.getEntity(), response.getEntity().getMediaType());
