...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Connector connector;
        ScriptEngine javaScriptEngine = ScriptEngineBuilder.create();

        if (commandLineArguments.haveSourceFiles()) {
            connector = new BatchClient(Protocol.HTTP, javaScriptEngine, commandLineArguments.getSourceFiles());
        } else {
            ...
            if (commandLineArguments.haveServer()) {
                connector = new Server(Protocol.HTTP, commandLineArguments.getServerPort(),
                        new InteractiveRestlet(javaScriptEngine, "server> "));
            } else {
                connector = new InteractiveClient(Protocol.HTTP, javaScriptEngine);
            }
        }
