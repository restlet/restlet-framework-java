...
/**
 * A method of a resource class annotated with a request method designator that is used to handle requests on the corresponding resource, see section 2.2.
 * 
...
public class ResourceMethod extends AbstractMethodWrapper {
	 ...
	 * @param javaMethod the Java method to wrap.
	 * @param path the path of the method.
	 * @param resourceClass the wrapped class of the method.
	 * @param httpMethod the HTTP method of the Java method. It will be checked be the {@link JaxRsRouter}, so avoiding double work. It will be requested from the javaMethod. 
	 */
	public ResourceMethod(Method javaMethod, Path path, ResourceClass resourceClass, org.restlet.data.Method httpMethod) {
		super(javaMethod, path, resourceClass);
		this.httpMethod = httpMethod;
	}

	/**
	 * Determine the media type if a produced entity.
	 * see JSR-311-Spec, Section 2.6 "Determining the MediaType of Responses"
	 * <br />
	...
	public MediaType getProducedMediaType(Object entity) {
		// LATER getProducedMediaType(.): see JSR-311-Spec, Section 2.6 "Determining the MediaType of Responses"
	...
	/**
	 * @return Returns an unmodifiable List of MediaTypes the given Resource Method or Resource Class produces. If no MediaType was given, a empty List will
	 *         returned. Will never return null.
	 */
	public List<MediaType> getProducedMimes() {
		if (producedMimes == null) {
			ProduceMime produceMime = this.javaMethod.getAnnotation(ProduceMime.class);
			if (produceMime == null)
	...
	/**
	 * @return Returns an unmodifiable List with the MediaTypes the given resourceMethod consumes. 
	 *         If no consumeMime is given, this method returns a List with MediaType.ALL.
	 *         Will never return null.
	 */
	public List<MediaType> getConsumedMimes() {
		if (this.consumedMimes == null) {
			ConsumeMime consumeMime = this.javaMethod.getAnnotation(ConsumeMime.class);
			if (consumeMime == null)
	 ...
	 * @param accMediaTypess The Media Types the client would accept, ordered by quality. See {@link Util#sortMetadataList(Collection)}
	 * @return Returns true, if the give MediaType is supported by the method, or no MediaType is given for the method, otherweise false.
	 */
	public boolean isAcceptedMediaTypeSupported(List<Collection<MediaType>> accMediaTypess) {
	 ...
	 * @return true, if the gien java method is annotated with a runtime designator for the given requested Method. If the requested method is null, than the
	 *         method returns true, when the method is annotated with any runtime desginator.
	...
	static org.restlet.data.Method getHttpMethod(Method javaMethod) {
		for (Annotation annotation : javaMethod.getAnnotations()) {
			...
			HttpMethod httpMethodAnnot = (HttpMethod) annoType.getAnnotation(HttpMethod.class);
			if (httpMethodAnnot != null) { // Annotation der Annotation der Methode ist HTTP-Methode
	 ...
	 * @return Returns true, if the given MediaType is supported by the method, or no MediaType is given for the method, otherweise false;
	 */
	...
	private static List<MediaType> convertToMediaTypes(String[] mimes) {
		List<MediaType> mediaTypes = new ArrayList(mimes.length);
	...
	{
		return this.getClass().getSimpleName()+"["+javaMethod.toString()+", "+this.httpMethod+"]";
	}
}