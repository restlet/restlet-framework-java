...
public class XdbServletWarClientHelper extends ServletWarClientHelper {
  ...
  /**
   * XMLDB base directory used to access to user resources
   * URL: file:///$HOME/myPath/myFile.ext
   * XDB: /home/USER/myPath/myFile.ext
   */
  private static final String HOME_DIR = "/$HOME";

  /**
   * XMLDB base directory used to access to deployed resources as WAR.
   * URL: war:///myPath/myFile.ext
   * XDB: /home/USER/wars/HelloRestlet/myPath/myFile.ext
   */
  private static final String USER_DIR = "/home/";

  /**
   * XMLDB path directory used to access to deployed resources as WAR.
   * URL: war:///myPath/myFile.ext
   * XDB: /home/USER/wars/HelloRestlet/myPath/myFile.ext
   */
  ...
  public void handle(Request request, Response response) {
    ...
    if (request.getMethod().equals(Method.GET) ||
      ...
      if (request.getProtocol().equals(Protocol.FILE)) {
        xdbResPath =
            (basePath.startsWith(HOME_DIR)) ? USER_DIR + this.connectedUser +
            basePath.substring(HOME_DIR.length()) : basePath;
      } else {
        xdbResPath =
            USER_DIR + this.connectedUser + DEPLOY_DIR + this.config.getServletName() +
            basePath;
      }
      ...
      if (basePath.endsWith("/")) {
        ...
        try {
          stmt =
              this.conn.prepareStatement("SELECT path(1),extractValue(res,'/Resource/@Container') " +
        ...
        try {
          ...
          if (sqe.getErrorCode() == 31001) {
            ...
            getLogger().throwing("XdbServletWarClientHelper", "handleWar: ",
                                 sqe);
