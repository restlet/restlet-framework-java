 ...
 * specifies the fully qualified name of a GWT module.  Requests directed to
 * the module will be forwarded to GWTShellServlet; Restlet will handle all
 * other requests.
 * <p>
...
public class GWTShellServletWrapper extends ServerServlet {
	...
	private void destroyGWTShellServlet(){
		try{
	    	gwtShellServlet.getClass().getMethod("destroy").invoke(gwtShellServlet);
		} catch (IllegalAccessException x) {
            log("[Noelios Restlet Engine] - Unable to destroy GWTShellServlet",x);
	...
	private void initGWTShellServlet(ServletConfig servletConfig){
		try{
	    	gwtShellServlet.getClass().getMethod("init", ServletConfig.class).invoke(gwtShellServlet,servletConfig);
		} catch (IllegalAccessException x) {
    ...
    private void instantiateGWTShellServlet(){
		try{
			gwtShellServlet = Class.forName("com.google.gwt.dev.shell.GWTShellServlet").newInstance();
			gwtShellServletServiceMethod = 
				gwtShellServlet.getClass().getMethod("service",ServletRequest.class,ServletResponse.class);
		} catch (IllegalAccessException x) {
            log("[Noelios Restlet Engine] - Unable to instantiate GWTShellServlet",x);
		} catch (ClassNotFoundException x) {
            log("[Noelios Restlet Engine] - Unable to instantiate GWTShellServlet",x);
		} catch (InstantiationException x) {
            log("[Noelios Restlet Engine] - Unable to instantiate GWTShellServlet",x);
		} catch (NoSuchMethodException x) {
            log("[Noelios Restlet Engine] - Unable to find service method for GWTShellServlet",x);
		}
	}
     ...
     */
	private void serviceGWTShellServlet(ServletRequest request, ServletResponse response){
		try{
	    	gwtShellServletServiceMethod.invoke(gwtShellServlet,request,response);
		} catch (IllegalAccessException x) {
            log("[Noelios Restlet Engine] - Unable to call service(request,response) in GWTShellServlet",x);
		} catch (InvocationTargetException x) {
            log("[Noelios Restlet Engine] - Unable to call service(request,response) in GWTShellServlet",x);
		}
	}
