...
public class ChallengeAuthenticator extends Authenticator {
	 ...
	 * provided (in general an identifier + secret couple). If the credentials are
	 * valid, the next Restlet attached is invoked.<br>
	 * <br>
	 * If the credentials are missing, then {@link #challenge(Response, boolean)} is
	 * invoked.<br>
	 * <br>
	 * If the credentials are invalid and if the "rechallenge" property is true then
	 * {@link #challenge(Response, boolean)} is invoked. Otherwise,
	 * {@link #forbid(Response)} is invoked.<br>
	 * <br>
	 * If the credentials are stale, then {@link #challenge(Response, boolean)} is
	 * invoked with the "stale" parameter to true.<br>
	 * <br>
	 * At the end of the process, the {@link ClientInfo#setAuthenticated(boolean)}
	 * method is invoked.
	 */
	@Override
	...
	/**
	 * Challenges the client by adding a challenge request to the response and by
	 * setting the status to {@link Status#CLIENT_ERROR_UNAUTHORIZED}.
	 * 
	...
	/**
	 * Rejects the call due to a failed authentication or authorization. This can be
	 * overridden to change the default behavior, for example to display an error
	 * page. By default, if authentication is required, the challenge method is
	 * invoked, otherwise the call status is set to CLIENT_ERROR_FORBIDDEN.
	 * 
	...
	public void forbid(Response response) {
		...
		if (loggable) {
			getLogger().log(Level.FINE,
					"Authentication or authorization failed for this URI: " + response.getRequest().getResourceRef());
		}
