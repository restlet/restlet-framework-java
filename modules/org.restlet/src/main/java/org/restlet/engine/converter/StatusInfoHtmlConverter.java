...
public class StatusInfoHtmlConverter extends ConverterHelper {
	...
	/**
	 * Returns the status information to display in the default representation. By
	 * default it returns the status's reason phrase.
	 * 
	...
	protected boolean isCompatible(Variant variant) {
		return (variant != null)
				&& (VARIANT_TEXT_HTML.isCompatible(variant) || VARIANT_APPLICATION_XHTML.isCompatible(variant));
	}
