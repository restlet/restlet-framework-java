...
public class HeaderUtils {
	...
	private static final Set<String> STANDARD_HEADERS = Collections
			.unmodifiableSet(new CaseInsensitiveHashSet(Arrays.asList(HEADER_ACCESS_CONTROL_ALLOW_CREDENTIALS,
					HEADER_ACCESS_CONTROL_ALLOW_HEADERS, HEADER_ACCESS_CONTROL_ALLOW_METHODS,
					HEADER_ACCESS_CONTROL_ALLOW_ORIGIN, HEADER_ACCESS_CONTROL_EXPOSE_HEADERS,
					HEADER_ACCESS_CONTROL_MAX_AGE, HEADER_ACCESS_CONTROL_REQUEST_HEADERS,
					HEADER_ACCESS_CONTROL_REQUEST_METHOD, HEADER_ACCEPT, HEADER_ACCEPT_CHARSET, HEADER_ACCEPT_ENCODING,
					HEADER_ACCEPT_LANGUAGE, HEADER_ACCEPT_PATCH, HEADER_ACCEPT_RANGES, HEADER_AGE, HEADER_ALLOW,
					HEADER_AUTHENTICATION_INFO, HEADER_AUTHORIZATION, HEADER_CACHE_CONTROL, HEADER_CONNECTION,
					HEADER_CONTENT_DISPOSITION, HEADER_CONTENT_ENCODING, HEADER_CONTENT_LANGUAGE, HEADER_CONTENT_LENGTH,
					HEADER_CONTENT_LOCATION, HEADER_CONTENT_MD5, HEADER_CONTENT_RANGE, HEADER_CONTENT_TYPE,
					HEADER_COOKIE, HEADER_DATE, HEADER_ETAG, HEADER_EXPECT, HEADER_EXPIRES, HEADER_FROM, HEADER_HOST,
					HEADER_IF_MATCH, HEADER_IF_MODIFIED_SINCE, HEADER_IF_NONE_MATCH, HEADER_IF_RANGE,
					HEADER_IF_UNMODIFIED_SINCE, HEADER_LAST_MODIFIED, HEADER_LOCATION, HEADER_MAX_FORWARDS,
					HEADER_PROXY_AUTHENTICATE, HEADER_PROXY_AUTHORIZATION, HEADER_RANGE, HEADER_REFERRER,
					HEADER_RETRY_AFTER, HEADER_SERVER, HEADER_SET_COOKIE, HEADER_SET_COOKIE2, HEADER_USER_AGENT,
	...
	private static final Set<String> UNSUPPORTED_STANDARD_HEADERS = Collections
			.unmodifiableSet(new CaseInsensitiveHashSet(Arrays.asList(HEADER_PRAGMA, HEADER_TRAILER,
					HEADER_TRANSFER_ENCODING, HEADER_TRANSFER_EXTENSION, HEADER_UPGRADE)));
	...
	public static void addEntityHeaders(Representation entity, Series<Header> headers) {
		...
		if (entity != null) {
			...
			if (entity.getDigest() != null && ALGORITHM_MD5.equals(entity.getDigest().getAlgorithm())) {
				addHeader(HEADER_CONTENT_MD5,
						new String(java.util.Base64.getEncoder().encode(entity.getDigest().getValue())), headers);
			}
			...
			if (entity.getModificationDate() != null) {
				addHeader(HEADER_LAST_MODIFIED, DateWriter.write(entity.getModificationDate()), headers);
			}
			...
			if (entity.getDisposition() != null && !TYPE_NONE.equals(entity.getDisposition().getType())) {
				addHeader(HEADER_CONTENT_DISPOSITION, DispositionWriter.write(entity.getDisposition()), headers);
			}
		}
	}
	...
	public static void addGeneralHeaders(Message message, Series<Header> headers) {
		addHeader(HEADER_CACHE_CONTROL, CacheDirectiveWriter.write(message.getCacheDirectives()), headers);
	...
	public static void addRequestHeaders(Request request, Series<Header> headers) {
		...
		if (!clientInfo.getAcceptedMediaTypes().isEmpty()) {
			addHeader(HEADER_ACCEPT, PreferenceWriter.write(clientInfo.getAcceptedMediaTypes()), headers);
		} else {
			addHeader(HEADER_ACCEPT, MediaType.ALL.getName(), headers);
		}

		if (!clientInfo.getAcceptedCharacterSets().isEmpty()) {
			addHeader(HEADER_ACCEPT_CHARSET, PreferenceWriter.write(clientInfo.getAcceptedCharacterSets()), headers);
		}

		if (!clientInfo.getAcceptedEncodings().isEmpty()) {
			addHeader(HEADER_ACCEPT_ENCODING, PreferenceWriter.write(clientInfo.getAcceptedEncodings()), headers);
		}

		if (!clientInfo.getAcceptedLanguages().isEmpty()) {
			addHeader(HEADER_ACCEPT_LANGUAGE, PreferenceWriter.write(clientInfo.getAcceptedLanguages()), headers);
		}

		if (!clientInfo.getAcceptedPatches().isEmpty()) {
			addHeader(HEADER_ACCEPT_PATCH, PreferenceWriter.write(clientInfo.getAcceptedPatches()), headers);
		}

		if (!clientInfo.getExpectations().isEmpty()) {
			addHeader(HEADER_EXPECT, ExpectationWriter.write(clientInfo.getExpectations()), headers);
		}
		...
		if (conditions.getRangeTag() != null && conditions.getRangeDate() != null) {
			Context.getCurrentLogger().log(WARNING,
		...
		if (request.getAccessControlRequestHeaders() != null) {
			addHeader(HEADER_ACCESS_CONTROL_REQUEST_HEADERS,
					StringWriter.write(request.getAccessControlRequestHeaders()), headers);
		}

		if (request.getAccessControlRequestMethod() != null) {
			addHeader(HEADER_ACCESS_CONTROL_REQUEST_METHOD, request.getAccessControlRequestMethod().getName(), headers);
		}
		...
		if (proxyChallengeResponse != null) {
			String authHeader = org.restlet.engine.security.AuthenticatorUtils.formatResponse(proxyChallengeResponse,
					request, headers);
	...
	public static void addResponseHeaders(Response response, Series<Header> headers) {
		...
		if (response.getProxyChallengeRequests() != null) {
			for (ChallengeRequest challengeRequest : response.getProxyChallengeRequests()) {
				addHeader(HEADER_PROXY_AUTHENTICATE, org.restlet.engine.security.AuthenticatorUtils
						.formatRequest(challengeRequest, response, headers), headers);
			}
		}
		...
		if (response.getChallengeRequests() != null) {
			for (ChallengeRequest challengeRequest : response.getChallengeRequests()) {
				addHeader(HEADER_WWW_AUTHENTICATE, org.restlet.engine.security.AuthenticatorUtils
						.formatRequest(challengeRequest, response, headers), headers);
			}
		}
		...
		if (response.getAccessControlAllowCredentials() != null) {
			addHeader(HEADER_ACCESS_CONTROL_ALLOW_CREDENTIALS, response.getAccessControlAllowCredentials().toString(),
					headers);
		}

		if (response.getAccessControlAllowHeaders() != null) {
			addHeader(HEADER_ACCESS_CONTROL_ALLOW_HEADERS, StringWriter.write(response.getAccessControlAllowHeaders()),
					headers);
		}
		...
		if (response.getAccessControlAllowMethods() != null) {
			addHeader(HEADER_ACCESS_CONTROL_ALLOW_METHODS, MethodWriter.write(response.getAccessControlAllowMethods()),
					headers);
		}
		if (response.getAccessControlExposeHeaders() != null) {
			addHeader(HeaderConstants.HEADER_ACCESS_CONTROL_EXPOSE_HEADERS,
					StringWriter.write(response.getAccessControlExposeHeaders()), headers);
		}
		if (response.getAccessControlMaxAge() > 0) {
			addHeader(HeaderConstants.HEADER_ACCESS_CONTROL_MAX_AGE,
					Integer.toString(response.getAccessControlMaxAge()), headers);
		}
		...
		if (response.getAuthenticationInfo() != null) {
			addHeader(HEADER_AUTHENTICATION_INFO, org.restlet.engine.security.AuthenticatorUtils
					.formatAuthenticationInfo(response.getAuthenticationInfo()), headers);
		}
				...
				if (HEADER_LOCATION.equalsIgnoreCase(header.getName())) {
					...
					try {
						...
						Context.getCurrentLogger().log(Level.WARNING,
								"Error during Age header parsing. Header: " + header.getValue(), nfe);
					}
					...
					if (retryAfter == null) {
						...
						try {
							...
							Context.getCurrentLogger().log(Level.WARNING,
									"Error during Retry-After header parsing. Header: " + header.getValue(), nfe);
						}
					}
					...
					try {
						...
						Context.getCurrentLogger().log(Level.WARNING,
								"Error during cookie setting parsing. Header: " + header.getValue(), e);
					}
				} else if (HEADER_WWW_AUTHENTICATE.equalsIgnoreCase(header.getName())) {
					List<ChallengeRequest> crs = org.restlet.engine.security.AuthenticatorUtils.parseRequest(response,
							header.getValue(), headers);
	 ...
	 */
	public static Representation extractEntityHeaders(Iterable<Header> headers, Representation representation)
			throws NumberFormatException {
		...
		if (headers != null) {
			for (Header header : headers) {
				if (HEADER_CONTENT_TYPE.equalsIgnoreCase(header.getName())) {
					...
					result.setDigest(new org.restlet.data.Digest(org.restlet.data.Digest.ALGORITHM_MD5,
	...
	/**
	 * Indicates if the given character is in ISO Latin 1 (8859-1) range. Note that
	 * this range is a superset of ASCII and a subrange of Unicode (UTF-8).
	 * 
	 * @param character The character to test.
	...
	public static boolean isLinearWhiteSpace(int character) {
		return (isCarriageReturn(character) || isSpace(character) || isLineFeed(character)
