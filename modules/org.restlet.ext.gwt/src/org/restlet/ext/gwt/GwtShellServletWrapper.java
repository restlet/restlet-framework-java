...
public class GwtShellServletWrapper extends ServerServlet {
    ...
    private void destroyGWTShellServlet() {
        try {
            ...
            log("[Restlet Framework] - Unable to destroy GWTShellServlet",
                    x);
    ...
    private void initGWTShellServlet(ServletConfig servletConfig) {
        try {
            ...
            log("[Restlet Framework] - Unable to init GWTShellServlet", x);
        } catch (InvocationTargetException x) {
            log("[Restlet Framework] - Unable to init GWTShellServlet", x);
        } catch (NoSuchMethodException x) {
            log("[Restlet Framework] - Unable to init GWTShellServlet", x);
    ...
    private void instantiateGWTShellServlet() {
        try {
            ...
            log(
                    "[Restlet Framework] - Unable to instantiate GWTShellServlet",
                    x);
            ...
            log(
                    "[Restlet Framework] - Unable to find service method for GWTShellServlet",
                    x);
        }
    }
    ...
    private void serviceGWTShellServlet(ServletRequest request,
        ...
        try {
            ...
            log(
                    "[Restlet Framework] - Unable to call service(request,response) in GWTShellServlet",
                    x);
        } catch (InvocationTargetException x) {
            log(
                    "[Restlet Framework] - Unable to call service(request,response) in GWTShellServlet",
