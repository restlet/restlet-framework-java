...
/**
 * Swagger 2.0 enabled application. This subclass of {@link Application} can
 ...
 * Swagger2Application#attachSwagger* methods<br>
 * 
 ...
 * of {@link Swagger2SpecificationRestlet}.
 * 
 * @author Thierry Boileau
 */
public class Swagger2Application extends Application implements
     ...
     * {@link Swagger2SpecificationRestlet} able to generate Swagger
     * documentation.
     * 
     * @param current
     *            The current Restlet to inspect.
    ...
    private static boolean isDocumented(Restlet current) {
        ...
        } else if (current instanceof Swagger2SpecificationRestlet) {
            documented = true;
        }
     ...
     *            The router on which defining the new route.
     * 
     * @see #attachSwagger2SpecificationRestlet(org.restlet.routing.Router,
     *      String) to attach it with a custom path
     */
    public void attachSwagger2SpecificationRestlet(Router router) {
        getSwagger2SpecificationRestlet(getContext()).attach(router);
        documented = true;
    }
     ...
     * 
     * @see #attachSwagger2SpecificationRestlet(org.restlet.routing.Router) to
     *      attach it with the default path
     */
    public void attachSwagger2SpecificationRestlet(Router router, String path) {
        getSwagger2SpecificationRestlet(getContext()).attach(router, path);
        documented = true;
    }

    /**
     * Overrides the parent's implementation. It checks that the application has
     * been documented using a {@link Swagger2SpecificationRestlet}. By default,
        ...
        if (!documented) {
            synchronized (this) {
                if (!documented) {
                    ...
                    if (!documented) {
                        attachSwagger2SpecificationRestlet(rootRouter);
                        documented = true;
                    }
                }
            }
     ...
     *         formats.
     */
    public Swagger2SpecificationRestlet getSwagger2SpecificationRestlet(
            Context context) {
        Swagger2SpecificationRestlet result = new Swagger2SpecificationRestlet(
                context);
