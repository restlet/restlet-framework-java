...
public class SpringComponent extends org.restlet.Component {
    ...
    /**
     * Adds a client to the list of connectors. The value can be either a protocol name, a Protocol
     * instance, or a Client instance.
     *
    ...
    public synchronized void setClientsList(List<Object> clients) {
        for (final Object client : clients) {
            switch (client) {
                case String string -> getClients().add(Protocol.valueOf(string));
                case Protocol protocol -> getClients().add(protocol);
                case Client client1 -> getClients().add(client1);
                case null, default ->
                        getLogger()
                                .warning(
    ...
    /**
     * Adds a server to the list of connectors. The value can be either a protocol name, a Protocol
     * instance, or a Server instance.
     *
     * @param serverInfo The server info.
     */
    public void setServer(Object serverInfo) {
        final List<Object> servers = new ArrayList<>();
     ...
     *
     * @param serversInfo The list of servers.
     */
    public void setServersList(List<Object> serversInfo) {
        for (final Object serverInfo : serversInfo) {
            switch (serverInfo) {
                case String string -> getServers().add(Protocol.valueOf(string));
                case Protocol protocol -> getServers().add(protocol);
                case Server server -> getServers().add(server);
                case null, default ->
                        getLogger()
                                .warning(
