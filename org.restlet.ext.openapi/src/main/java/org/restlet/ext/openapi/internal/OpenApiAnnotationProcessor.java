...
public class OpenApiAnnotationProcessor {
    ...
    public static void documentOperation(
            Operation operation, io.swagger.v3.oas.annotations.Operation operationAnnotation) {
        if (operationAnnotation.summary() != null && !operationAnnotation.summary().isEmpty()) {
            operation.setSummary(operationAnnotation.summary());
        }

        if (operationAnnotation.description() != null
                && !operationAnnotation.description().isEmpty()) {
            operation.setDescription(operationAnnotation.description());
        }

        if (operationAnnotation.parameters() != null) {
            for (io.swagger.v3.oas.annotations.Parameter parameterAnnotation :
    ...
    public static void documentOperationResponse(
        ...
        io.swagger.v3.oas.models.responses.ApiResponse apiResponse =
                new io.swagger.v3.oas.models.responses.ApiResponse()
                        .description(
                                isNullOrEmpty(apiResponseAnnotation.description())
                                        ? defaultDescription
                                        : apiResponseAnnotation.description());

        if (!isContentEmpty(apiResponseAnnotation)) {
            AnnotationsUtils.getContent(
                            apiResponseAnnotation.content(), null, null, null, null, null)
    ...
    private static Optional<Parameter> resolveParameterFromAnnotation(
            ...
            case COOKIE, HEADER, QUERY -> {
                Parameter parameter =
                        new Parameter()
                                ...
                                .description(
                                        isNullOrEmpty(parameterAnnotation.description())
                                                ? null
    ...
    private static boolean isContentEmpty(ApiResponse apiResponseAnnotation) {
        if (apiResponseAnnotation.content() == null
                || apiResponseAnnotation.content().length == 0) {
            return true;
        }
