 ...
 * the dot-segments. Note that applying the getRelativeRef() method on an
 * absolute reference returns the current reference relatively to a base
 ...
 * URI data. Our idea is to protect encodings and special characters in all
 * cases and reduce the memory size taken by this class while making Reference
 * instances mutable.
 * </p>
 * <p>
...
public class Reference {
	 ...
	 * one and adds a slash in front of the given last segment. <br>
	 * Note that no URI decoding is done by this method.
	 * 
			...
			if (oldPart.startsWith("//")) {
				...
				if (indexSlash != -1) {
					if (indexQuery != -1) {
						// Query found
						if (indexSlash < indexQuery) {
							newPart = oldPart.substring(0, indexSlash) + path + oldPart.substring(indexQuery);
						} else {
							// '/' inside the query string: no path found
							newPart = oldPart.substring(0, indexQuery) + path + oldPart.substring(indexQuery);
						}
					} else {
						// No query found
	...
	public void setQuery(String query) {
		...
		if (hasQuery()) {
			...
			if (hasFragment()) {
				// Fragment found
				if (!emptyQueryString) {
					setInternalRef(this.internalRef.substring(0, this.fragmentIndex) + '?' + query
							+ this.internalRef.substring(this.fragmentIndex));
				} else {
					// Do nothing
				}
	...
	public void setSchemeSpecificPart(String schemeSpecificPart) {
		...
		if (hasScheme()) {
			...
			if (hasFragment()) {
				// Fragment found
				setInternalRef(this.internalRef.substring(0, this.schemeIndex + 1) + schemeSpecificPart
						+ this.internalRef.substring(this.fragmentIndex));
			} else {
				// No fragment found
				setInternalRef(this.internalRef.substring(0, this.schemeIndex + 1) + schemeSpecificPart);
