 ...
 * 
 * @see <a href=
 *      "https://datatracker.ietf.org/doc/html/rfc2617#section-3.2.3">HTTP
...
public class AuthenticationInfo {
	...
	/**
	 * Returns the next server nonce. This is the nonce the server wishes the client
	 ...
	 * {@link ChallengeMessage#QUALITY_AUTHENTICATION_INTEGRITY} for authentication
	 ...
	 * when used with HTTP connectors, this property maps to the "response-digest"
	 * value in the "response-auth" directive of the "Authentication-Info" header.
	 * 
	...
	@Override
	public int hashCode() {
		return SystemUtils.hashCode(getNextServerNonce(), getNonceCount(), getClientNonce(), getQuality(),
				getResponseDigest());
	}
	...
	/**
	 * Sets the next server nonce. This is the nonce the server wishes the client to
	 * use for a future authentication response
	 * 
	...
	/**
	 * Sets the optional response digest for mutual authentication. Note that when
	 * used with HTTP connectors, this property maps to the "response-digest" value
	 * in the "response-auth" directive of the "Authentication-Info" header.
	 * 
