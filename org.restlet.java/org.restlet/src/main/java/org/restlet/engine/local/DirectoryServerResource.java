 ...
 * 
 * @see <a href=
 *      "https://httpd.apache.org/docs/current/content-negotiation.html">Apache
...
public class DirectoryServerResource extends ServerResource {
	 ...
	 * <ul>
	 * <li>the underlying helper may not support content negotiation and be able to
	 * return the list of possible variants of the target file (e.g. the CLAP
	 * helper).</li>
	 * <li>the underlying helper may not support directory listing</li>
	 * <li>the extensions tunneling cannot apply on a directory</li>
	 * <li>underlying helpers that do not support content negotiation cannot support
	 * extensions tunneling</li>
	 * </ul>
	 */
	@Override
	public void doInit() throws ResourceException {
		...
		if (contextResponse.getEntity() != null) {
			...
			// directory listing with the media-type "MediaType.TEXT_URI_LIST" when handling
			// directories
			...
			if (this.targetUri.endsWith("/")) {
				...
				// Try to determine if this target URI with no trailing "/" is a directory, in
				// order to force the
				// redirection.
				if (!StringUtils.isNullOrEmpty(getDirectory().getIndexName())) {
					// Append the index name
		...
		if (isNegotiated() && !this.directoryTarget && !this.fileTarget && this.originalRef != null) {
			...
			// The target URI does not take into account the query and fragment parts of the
			// resource.
		...
		if (!fileTarget || fileContent == null || !getRequest().getMethod().isSafe()) {
			// Try to get the directory content, in case the request does not target a
			// directory
	 ...
	 * It completes the request's attributes map with the current Directory
	 * ("org.restlet.directory" key).
	 *
