...
public class TemplateRoute extends Route {
	...
	public TemplateRoute(Router router, String uriTemplate, Restlet next) {
		this(router, new Template(uriTemplate, Template.MODE_STARTS_WITH, Variable.TYPE_URI_SEGMENT, "", true, false),
				next);
	}
	 ...
	 * reference with the template.
	 * 
	 * @return True if the query part of the reference should be taken into account,
	 *         false otherwise.
	 */
				...
				if (matchedLength != -1) {
					...
					if (totalLength > 0.0F) {
						result = getRouter().getRequiredScore()
								+ (1.0F - getRouter().getRequiredScore()) * (matchedLength / totalLength);
					} else {
						result = 1.0F;
					}
				}
	 ...
	 * string.
	 * 
	 * @param matchingQuery True if the matching should be done with the query
