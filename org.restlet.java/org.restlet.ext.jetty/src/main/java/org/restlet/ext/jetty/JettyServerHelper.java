 ...
 * <td>int</td>
 * <td>0</td>
 * <td>Low-resource monitor max connections; when 0, the check is disabled.
 * Deprecated, see server.maxConnections</td>
 * </tr>
 * <tr>
 ...
 * allowed for the service to shutdown. Deprecated, use shutdown.timeout
 * instead</td>
 * </tr>
 * <tr>
 ...
 * </table>
 * 
 * @see <a href= "https://jetty.org/docs/jetty/12/index.html">Jetty 12
 *      documentation</a>
...
public abstract class JettyServerHelper extends org.restlet.engine.adapter.HttpServerHelper {
	...
	/**
	 * Connector acceptor thread count. Defaults to -1. When -1, Jetty will default
	 * to 1.
	 * 
	...
	/**
	 * Connector executor. Defaults to null. When null, will use the server's thread
	 * pool.
	 * 
	...
	/**
	 * Connector selector thread count. Defaults to -1. When less or equal than 0,
	 * Jetty computes a default value derived from a heuristic over available CPUs
	 ...
	 * <p>
	 * A larger buffer can improve performance by allowing a content producer to run
	 * without blocking, however, larger buffers consume more memory and may induce
	...
	/**
	 * Low-resource monitor max memory in bytes. Defaults to 0. When 0, the check is
	 * disabled.
	 * <p>
	...
	/**
	 * Low-resource monitor, whether to check if we're low on threads. Defaults to
	 * true.
	 * 
	...
	/**
	 * The endpoint idle timeout in milliseconds to apply when the connection limit
	 * is reached. Defaults to 0. When 0, there is no idle timeout.
	 * <p>
	 * The maximum time allowed for the endpoint to close when the connection limit
	 * is reached.
	 *
	 * @return The endpoint idle timeout in milliseconds to apply when the
	...
	/**
	 * When true, upon JVM shutdown, the Jetty server will block incoming requests
	 * and wait for pending requests to end before shutting down. Otherwise,
	 * incoming requests are not blocked and all requests are aborted. Defaults to
	 * true.
	 * 
	 * @return True if upon JVM shutdown, the Jetty server will block incoming
	 *         requests and wait for pending requests to end before shutting down.
	 *         Otherwise, incoming requests are not blocked and all requests are
	 *         aborted.
	 */
