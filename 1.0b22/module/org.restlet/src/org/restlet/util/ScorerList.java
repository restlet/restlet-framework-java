...
/**
 * Modifiable list of scorers with some helper methods. Note that this class
 * implements the java.util.List interface using the Scorer interface as the
...
public final class ScorerList extends WrapperList<Scorer> {
    ...
    /**
     * Creates then adds a scorer at the end of the list. Adds a target option
    ...
    /**
     * Creates then adds a scorer based on an URI pattern at a specific
     * position.
     * 
     * @param uriPattern
     *            The URI pattern used to map calls (see
    ...
    public synchronized ScorerList subList(int fromIndex, int toIndex) {
        return new ScorerList(this.router, getDelegate().subList(fromIndex,
                toIndex));
    }
}
