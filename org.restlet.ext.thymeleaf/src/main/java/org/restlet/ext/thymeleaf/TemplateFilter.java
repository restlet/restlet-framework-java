...
/**
 * Filters response's entity and wraps it with a Thymeleaf's template representation. By default,
 * the template representation provides a data model based on the request and response objects. In
 * order for the wrapping to happen, the representations must have the {@link #THYMELEAF} encoding
 * set.<br>
 * <br>
...
public abstract class TemplateFilter extends Filter {
    ...
    private static final Encoding THYMELEAF =
            new Encoding("thymeleaf", "Thymeleaf templated representation");
     ...
     */
    protected TemplateFilter(Context context) {
        super(context);
    }
     ...
     */
    protected TemplateFilter(Context context, Restlet next) {
        super(context, next);
     ...
     * @param context The context.
     * @param next The next Restlet.
     * @param dataModel The filter's data model.
     */
    protected TemplateFilter(Context context, Restlet next, Map<String, Object> dataModel) {
        super(context, next);
     ...
     * @param dataModel The filter's data model.
     */
    protected TemplateFilter(Context context, Restlet next, Resolver<Object> dataModel) {
        super(context, next);
        this.mapDataModel = null;
    ...
    protected void afterHandle(Request request, Response response) {
        if (response.isEntityAvailable()
            ...
            final TemplateRepresentation representation =
                    new TemplateRepresentation(
                            (TemplateRepresentation) response.getEntity(),
                            getLocale(),
                            response.getEntity().getMediaType());
