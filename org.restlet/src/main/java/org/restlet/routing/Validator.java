...
public class Validator extends Filter {
     ...
     * template variable, adjust the base reference, then extracts the attributes from form
     * parameters (query, cookies, entity) and finally tries to validate the variables as indicated
     * by the {@link #validate(String, boolean, String)} method.
     *
    ...
    private List<ValidateInfo> getValidations() {
        ...
        if (v == null) {
            synchronized (this) {
                v = this.validations;
                if (v == null) {
                    this.validations = v = new CopyOnWriteArrayList<>();
                }
            }
        }
        return v;
    ...
    /**
     * Checks the request attributes for presence or format. If the check fails, then a response
     * status CLIENT_ERROR_BAD_REQUEST is returned with the proper status description.
     *
     * @param attribute Name of the attribute to look for.
     * @param required Indicates if the attribute presence is required.
     * @param format Format of the attribute value, using Regex pattern syntax.
    ...
    /**
     * Checks the request attributes for format only. If the check fails, then a response status
     * CLIENT_ERROR_BAD_REQUEST is returned with the proper status description.
     *
     * @param attribute Name of the attribute to look for.
     * @param format Format of the attribute value, using Regex pattern syntax.
     */
    ...
    /**
     * Checks the request attributes for presence only. If the check fails, then a response status
     * CLIENT_ERROR_BAD_REQUEST is returned with the proper status description.
     *
     * @param attribute Name of the attribute to look for.
     */
