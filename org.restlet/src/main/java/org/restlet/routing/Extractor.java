...
/**
 * Filter extracting attributes from a call. Multiple extractions can be defined, based on the query
 * string of the resource reference, on the request form (ex: posted from a browser) or on cookies.
 * <br>
 * <br>
...
public class Extractor extends Filter {
    ...
    /**
     * Allows filtering before its handling by the target Restlet. By default, it extracts the
     * attributes from form parameters (query, cookies, entity) and finally puts them in the
     * request's attributes ( {@link Request#getAttributes()}).
     *
     ...
     * @param first Indicates if only the first cookie should be set. Otherwise, as a List instance
     *     might be set in the attribute value.
     */
    ...
    private List<ExtractInfo> getCookieExtracts() {
        ...
        if (ce == null) {
            synchronized (this) {
                ...
                if (ce == null) {
                    this.cookieExtracts = ce = new CopyOnWriteArrayList<>();
                }
            }
        }
        return ce;
    ...
    private List<ExtractInfo> getEntityExtracts() {
        ...
        if (ee == null) {
            synchronized (this) {
                ...
                if (ee == null) {
                    this.entityExtracts = ee = new CopyOnWriteArrayList<>();
                }
            }
        }
        return ee;
    ...
    private List<ExtractInfo> getQueryExtracts() {
        ...
        if (qe == null) {
            synchronized (this) {
                ...
                if (qe == null) {
                    this.queryExtracts = qe = new CopyOnWriteArrayList<>();
                }
            }
        }
        return qe;
