...
/**
 * Rewrites URIs then redirects the call or the client to a new destination. There are various
 * redirection modes that you can choose from: client-side redirections ({@link #MODE_CLIENT_FOUND},
 * {@link #MODE_CLIENT_PERMANENT}, {@link #MODE_CLIENT_SEE_OTHER}, {@link #MODE_CLIENT_TEMPORARY})
 * or server-side redirections, similar to a reverse proxy ( {@link #MODE_SERVER_OUTBOUND} and
 * {@link #MODE_SERVER_INBOUND}).<br>
 * <br>
 * When setting the redirection URIs, you can also use special URI variables to reuse most
 * properties from the original request as well as URI template variables. For a complete list of
 * properties, please see the {@link Resolver} class. For example, "/target?referer={fi}" would
 * redirect to the relative URI, inserting the referrer URI as a query parameter.<br>
 * <br>
 * To create a reverse proxy, a typical configuration will use the {@link #MODE_SERVER_OUTBOUND}
 * constant and a target URI like "http://targetHost/targetRootPath/{rr}" to ensure that all child
 * URIs are properly redirected as well, "rr" appending the remaining part of the current request
 * URI that hasn't been routed yet.<br>
 * <br>
...
public class Redirector extends Restlet {
    /**
     * In this mode, the client is simply redirected to the URI generated from the target URI
     * pattern using the {@link Status#REDIRECTION_FOUND} status. Note: this is a client-side
     * redirection.<br>
     *
     ...
     * pattern, using the {@link Status#REDIRECTION_PERMANENT} status. Note: this is a client-side
     * redirection.<br>
     *
    ...
    /**
     * In this mode, the client is simply redirected to the URI generated from the target URI
     * pattern using the {@link Status#REDIRECTION_SEE_OTHER} status. Note: this is a client-side
     * redirection.<br>
     *
     ...
     * pattern using the {@link Status#REDIRECTION_TEMPORARY} status. Note: this is a client-side
     * redirection.<br>
     *
    ...
    /**
     * In this mode, the call is sent to {@link Context#getServerDispatcher()}. Once the selected
     * client connector has completed the request handling, the response is normally returned to the
     * client. In this case, you can view the Redirector as acting as a transparent proxy Restlet.
     * Note: this is a server-side redirection.<br>
     * <br>
     * Warning: remember to add the required connectors to the parent {@link Component} and to
     * declare them in the list of required connectors on the {@link
     * Application#getConnectorService()} property.<br>
     * <br>
     * Note that in this mode, the headers of HTTP requests, stored in the request's attributes, are
     * removed before dispatching. Also, when an HTTP response comes back, the headers are also
     * removed. You can control this behavior by setting the {@link #headersCleaning} attribute or
     * by overriding the {@link #rewrite(Request)} or {@link #rewrite(Response)}.
     *
    ...
    /**
     * In this mode, the call is sent to {@link Application#getOutboundRoot()} or if null to {@link
     * Context#getClientDispatcher()}. Once the selected client connector has completed the request
     * handling, the response is normally returned to the client. In this case, you can view the
     * {@link Redirector} as acting as a transparent server-side proxy. Note: this is a server-side
     * redirection.<br>
     * <br>
     * Warning: remember to add the required connectors to the parent {@link Component} and to
     * declare them in the list of required connectors on the {@link
    ...
    /**
     * Indicates if all headers of HTTP requests stored in the request's attributes must be removed
     * before the redirection. If set to true, it removes all headers, otherwise it keeps only the
     * extension (or non-HTTP standard) headers
     */
     ...
     * Returns null in case it cannot compute the new reference.
     *
        ...
        switch (this.mode) {
            case MODE_CLIENT_PERMANENT:
                if (request.isLoggable()) {
                    getLogger().fine(() -> "Permanently redirecting client to: " + targetRef);
                }
            ...
            case MODE_CLIENT_FOUND:
                if (request.isLoggable()) {
                    getLogger().fine(() -> "Redirecting client to found location: " + targetRef);
                }
            ...
            case MODE_CLIENT_SEE_OTHER:
                if (request.isLoggable()) {
                    getLogger().fine(() -> "Redirecting client to another location: " + targetRef);
                }
            ...
            case MODE_CLIENT_TEMPORARY:
                if (request.isLoggable()) {
                    getLogger().fine(() -> "Temporarily redirecting client to: " + targetRef);
                }
            ...
            case MODE_SERVER_OUTBOUND:
                if (request.isLoggable()) {
                    getLogger().fine(() -> "Redirecting via client dispatcher to: " + targetRef);
                }
            ...
            case MODE_SERVER_INBOUND:
                if (request.isLoggable()) {
                    getLogger().fine(() -> "Redirecting via server dispatcher to: " + targetRef);
                }
     ...
     * headers, stored in the request's attributes, are removed before dispatching. After
     * dispatching, the response HTTP headers are also removed to prevent conflicts with the main
     * call.
     *
     ...
     * {@link #MODE_SERVER_OUTBOUND} modes. By default, it just returns the initial entity without
     * any modification.
     *
    ...
    /**
     * Optionally updates the request sent in the {@link #MODE_SERVER_INBOUND} and {@link
     * #MODE_SERVER_OUTBOUND} modes. By default, it leverages the {@link #headersCleaning} attribute
     * to clean the headers: if set to true, it removes all headers, otherwise it keeps only the
     * extension (or non-HTTP standard) headers<br>
     *
    ...
    /**
     * Optionally updates the response sent in the {@link #MODE_SERVER_INBOUND} and {@link
     * #MODE_SERVER_OUTBOUND} modes. By default, it leverages the {@link #headersCleaning} attribute
     * to clean the headers: if set to true, it removes all headers, otherwise it keeps only the
     * extension (or non-HTTP standard) headers<br>
     *
