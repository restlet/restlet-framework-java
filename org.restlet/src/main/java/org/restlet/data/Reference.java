 ...
 * </pre>
 *
 * <p>Note that this class doesn't encode or decode the reserved characters. It assumes that the
 * URIs or the URI parts passed in are properly encoded using the standard URI encoding mechanism.
 * You can use the static "encode()" and "decode()" methods for this purpose. Note that if an
 * invalid URI character is detected by the constructor or one of the setters, a trace will be
 * logged and the character will be automatically encoded.
 *
 ...
 * segments "." and "..").
 *
 * <p>You can also apply the getTargetRef() method on absolute references to solve the dot-segments.
 * Note that applying the getRelativeRef() method on an absolute reference returns the current
 * reference relatively to a base reference, if any, and solves the dot-segments.
 *
 * <p>The Reference stores its data as a single string, the one passed to the constructor. This
 * string can always be obtained using the toString() method. A couple of integer indexes are
 * maintained to improve the extraction time of various reference properties (URI components).
 *
 * <p>When you modify a specific component of the URI reference, via the setPath() method, for
 * example, the internal string is simply regenerated by updating only the relevant part. We try as
 * much as possible to protect the bytes given to the Reference class instead of transparently
 * parsing and normalizing the URI data. Our idea is to protect encodings and special characters in
 * all cases and reduce the memory size taken by this class while making Reference instances
 * mutable.
 *
 * <p>Because the base reference is only a property of the Reference ("baseRef"). When you use the
 * "Reference(base, path)" constructor, it is equivalent to doing:<br>
 * ref = new Reference(path);<br>
 * ref.setBaseRef(base);
 *
 * <p>The base ref is not automatically resolved or "merged" with the rest of the reference
 * information (the path here). For example, this lets you reuse a single reference as the base of
 * several relative references. If you modify the base reference, all relative references are still
 * accurate. Note that the name and value properties are thread-safe, stored in volatile members.
 *
...
public class Reference {
    ...
    /**
     * Decodes a given string using the standard URI encoding mechanism. If the provided character
     * set is null, the string is returned but not decoded. <em><strong>Note:</strong> The <a
     * href="http://www.w3.org/TR/html40/appendix/notes.html#non-ascii-chars">World Wide Web
    ...
    public static String decode(String toDecode, CharacterSet characterSet) {
        ...
        try {
            result =
                    (characterSet == null)
                            ? toDecode
                            : java.net.URLDecoder.decode(toDecode, characterSet.getName());
    ...
    public static String encode(String toEncode, boolean queryString, CharacterSet characterSet) {
        ...
        try {
            result =
                    (characterSet == null)
                            ? toEncode
                            : java.net.URLEncoder.encode(toEncode, characterSet.getName());
        } catch (UnsupportedEncodingException uee) {
            Context.getCurrentLogger()
                    .log(
                            Level.WARNING,
    ...
    /**
     * Encodes a given string using the standard URI encoding mechanism. If the provided character
     * set is null, the string is returned but not encoded.
     *
     ...
     *
     * @param character The character to test.
     * @return True if the given character is a generic URI component delimiter character.
     */
     ...
     *
     * @param character The character to test.
     * @return True if the given character is a URI subcomponent delimiter character.
     */
     ...
     */
    public static String toString(
            String scheme, String host, String path, String query, String fragment) {
        final StringBuilder sb = new StringBuilder();
    ...
    /**
     * Adds a parameter to the query component. The name and value are automatically URL encoded if
     * necessary.
     *
    ...
    /**
     * Adds several parameters to the query component. The name and value are automatically URL
     ...
     *
     * @param decode Indicates if the result should be decoded using the {@link #decode(String)}
     *     method.
    ...
    /**
     * Returns the optional extensions for hierarchical identifiers. An extensions part starts after
     ...
     *
     * @return The optional port number for server-based hierarchical identifiers or -1 if the port
     ...
     * @param decode Indicates if the result should be decoded using the {@link #decode(String)}
     *     method.
     * @param excludeMatrix True if the matrix parameters are dropped from the segments.
     * @return The optionally decoded last segment.
     * @see #getLastSegment()
     */
    ...
    /**
     * Returns the parent reference of a hierarchical reference. The last slash of the path will be
     * considered as the end of the parent path.
     *
    ...
    /**
     * Returns the optionally decoded path component. If no path is available, it returns null.
     *
     * @param decode Indicates if the result should be decoded using the {@link #decode(String)}
     *     method.
    ...
    /**
     * Returns the current reference as a relative reference to the current base reference. This
     * method should only be invoked for absolute references, otherwise an IllegalArgumentException
     * will be raised.
     *
     * @return The current reference as a relative reference to the current base reference.
    ...
    /**
     * Returns the current reference relatively to a base reference. This method should only be
     ...
     *
     * <ul>
     *   <li>the reference is absolute
     *   <li>the reference identifier starts with the resource baseRef
     * </ul>
     *
     * <br>
     * Note that no URI decoding is done by this method.
    ...
    /**
     * Returns the optionally decoded remaining part with or without the query part of the
     * reference.
     *
     * @param decode Indicates if the result should be decoded using the {@link #decode(String)}
     *     method.
    ...
    /**
     * Returns true if both references are equivalent, meaning that they resolve to the same target
     * reference.
     *
    ...
    /**
     * Sets the extensions for hierarchical identifiers. An extensions part starts after the first
        ...
        if (authority == null) {
            ...
            if (indexUI != -1) {
                ...
                if (indexP != -1) {
                    // Port found
                    setAuthority(
                            authority.substring(0, indexUI + 1)
                                    + domain
    ...
    /**
     * Converts to a {@link java.net.URI} instance. Note that relative references are resolved
     * before conversion using the {@link #getTargetRef()} method.
     *
    ...
    /**
     * Converts to a {@link java.net.URL} instance. Note that relative references are resolved
     * before conversion using the {@link #getTargetRef()} method.
     *
