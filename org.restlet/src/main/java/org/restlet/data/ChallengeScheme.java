...
public final class ChallengeScheme {
    ...
    public static final ChallengeScheme CUSTOM =
            new ChallengeScheme("CUSTOM", "Custom", "Custom authentication");

    /** Plain FTP scheme. */
    public static final ChallengeScheme FTP_PLAIN =
            new ChallengeScheme("FTP_PLAIN", "PLAIN", "Plain FTP authentication");

    /** Amazon Query String HTTP scheme. */
    public static final ChallengeScheme HTTP_AWS_IAM =
            new ChallengeScheme("HTTP_AWS_IAM", "AWS3", "Amazon IAM-based authentication");
    ...
    public static final ChallengeScheme HTTP_AWS_S3 =
            new ChallengeScheme("HTTP_AWS_S3", "AWS", "Amazon S3 HTTP authentication");
     ...
     *
     * @see <a href= "http://msdn.microsoft.com/en-us/library/dd179428.aspx#Subheading2">MSDN
     *     page</a>
     */
    ...
    public static final ChallengeScheme HTTP_BASIC =
            new ChallengeScheme("HTTP_BASIC", "Basic", "Basic HTTP authentication");

    /** Cookie HTTP scheme. */
    public static final ChallengeScheme HTTP_COOKIE =
            new ChallengeScheme("HTTP_Cookie", "Cookie", "Cookie HTTP authentication");

    /** Digest HTTP scheme. */
    public static final ChallengeScheme HTTP_DIGEST =
            new ChallengeScheme("HTTP_DIGEST", "Digest", "Digest HTTP authentication");

    /** Microsoft NTML HTTP scheme. */
    public static final ChallengeScheme HTTP_NTLM =
            new ChallengeScheme("HTTP_NTLM", "NTLM", "Microsoft NTLM HTTP authentication");
    ...
    public static final ChallengeScheme HTTP_OAUTH =
            new ChallengeScheme("HTTP_OAuth", "OAuth", "OAuth 1.0 authentication");

    /** OAuth Bearer HTTP scheme. */
    public static final ChallengeScheme HTTP_OAUTH_BEARER =
            new ChallengeScheme("HTTP_Bearer", "Bearer", "OAuth 2.0 bearer token authentication");

    /** OAuth MAC HTTP scheme. */
    public static final ChallengeScheme HTTP_OAUTH_MAC =
            new ChallengeScheme(
                    "HTTP_MAC", "Mac", "OAuth 2.0 message authentication code authentication");

    /** Private list of schemes for optimization purpose. */
    private static final Map<String, ChallengeScheme> SCHEMES =
            Map.ofEntries(
                    Map.entry(CUSTOM.getName().toLowerCase(), CUSTOM),
                    Map.entry(FTP_PLAIN.getName().toLowerCase(), FTP_PLAIN),
                    Map.entry(HTTP_AWS_IAM.getName().toLowerCase(), HTTP_AWS_S3),
                    Map.entry(HTTP_AWS_QUERY.getName().toLowerCase(), HTTP_AWS_S3),
                    Map.entry(HTTP_AWS_S3.getName().toLowerCase(), HTTP_AWS_S3),
                    Map.entry(HTTP_AZURE_SHAREDKEY.getName().toLowerCase(), HTTP_AZURE_SHAREDKEY),
                    Map.entry(
                            HTTP_AZURE_SHAREDKEY_LITE.getName().toLowerCase(),
                            HTTP_AZURE_SHAREDKEY_LITE),
                    Map.entry(HTTP_BASIC.getName().toLowerCase(), HTTP_BASIC),
                    Map.entry(HTTP_COOKIE.getName().toLowerCase(), HTTP_COOKIE),
                    Map.entry(HTTP_DIGEST.getName().toLowerCase(), HTTP_DIGEST),
                    Map.entry(HTTP_NTLM.getName().toLowerCase(), HTTP_NTLM),
                    Map.entry(HTTP_OAUTH.getName().toLowerCase(), HTTP_OAUTH),
                    Map.entry(HTTP_OAUTH_BEARER.getName().toLowerCase(), HTTP_OAUTH),
                    Map.entry(HTTP_OAUTH_MAC.getName().toLowerCase(), HTTP_OAUTH));

    /**
     * Returns the challenge scheme associated with a scheme name. If an existing constant exists,
     ...
     */
    public ChallengeScheme(
            final String name, final String technicalName, final String description) {
        this.name = name;
    ...
    public boolean equals(final Object obj) {
        ...
        if (!(obj instanceof ChallengeScheme that)) {
            return false;
        }
