...
/**
 * Restlet managing a coherent set of resources and services. Applications are guaranteed to receive
 * calls with their base reference set relatively to the {@link VirtualHost} that served them. This
 * class is both a descriptor able to create the root Restlet and the actual Restlet that can be
 * attached to one or more VirtualHost instances.<br>
 * <br>
 * Applications also have many useful services associated. Most are enabled by default and are
 * available as properties that can be eventually overridden:
 *
 * <ul>
 *   <li>"connectorService" to declare necessary client and server connectors.
 *   <li>"converterService" to convert between regular objects and representations.
 *   <li>"decoderService" to automatically decode or uncompress received entities.
 *   <li>"encoderService" to automatically encode or compress sent entities (disabled by default).
 *   <li>"metadataService" to provide access to metadata and their associated extension names.
 *   <li>"rangeService" to automatically exposes ranges of response entities.
 *   <li>"statusService" to provide common representations for exception status.
 *   <li>"taskService" to run tasks asynchronously (disabled by default).
 *   <li>"tunnelService" to tunnel method names or client preferences via query parameters.
 * </ul>
 *
...
public class Application extends Restlet {
    ...
    /**
     * This variable is stored internally as a thread local variable and updated each time a call
     * enters an application.
     *
     * <p>Warning: this method should only be used under duress. You should by default prefer
     * getting the current application using methods such as {@link
     * org.restlet.resource.Resource#getApplication()}
     *
    ...
    /**
     * Constructor. Note this constructor is convenient because you don't have to provide a context
     * like for {@link #Application(Context)}. Therefore, the context will initially be null. It's
     * only when you attach the application to a virtual host via one of its attach*() methods that
     ...
     * Constructor.
     *
     * @param context The context to use based on parent component context. This context should be
     *     created using the {@link Context#createChildContext()} method to ensure proper isolation
    ...
    public Application(Context context) {
        ...
        this.services.add(
                new StatusService(true, converterService, metadataService, connegService));
    ...
    /**
     * Creates an inbound root Restlet that will receive all incoming calls. In general, instances
     * of Router, Filter, or Finder classes will be used as initial application Restlet. The default
     * implementation returns null by default. This method is intended to be overridden by
     * subclasses.
     *
    ...
    /**
     * Creates an outbound root Restlet that will receive all outgoing calls from ClientResource. In
     * general, instances of {@link Router} and {@link Filter} classes will be used. The default
     * implementation returns a Restlet giving access to the outbound service layer and finally to
     * the {@link Context#getClientDispatcher()}.
     *
     * <p>This method is intended to be overridden by subclasses, but to benefit from the outbound
     * service filtering layer, the original outbound root must be carefully attached again at the
     * end of the user filtering layer.
     *
    ...
    /**
     * Returns the role associated with the given name.
     *
    ...
    /**
     * Sets the modifiable list of roles. This method clears the current list and adds all entries
    ...
    /**
     * Starts the application, all the enabled associated services then the inbound and outbound
     * roots.
     */
    @Override
        ...
        if (isStopped()) {
            if (isDebugging()) {
                getLogger()
                        .log(
                                Level.INFO,
                                "Starting {0} application in debug mode",
                                getClass().getName());
            } else {
                getLogger().log(Level.INFO, "Starting {0} application", getClass().getName());
            }
    ...
    /**
     * Stops the application, the inbound and outbound roots, then all the enabled associated
     * services. Finally, it clears the internal cache of annotations.
     */
    @Override
