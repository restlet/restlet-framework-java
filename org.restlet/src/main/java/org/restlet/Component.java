...
/**
 * Restlet managing a set of {@link Connector}s, {@link VirtualHost}s, {@link Service}s and {@link
 * Application}s. Applications are expected to be directly attached to virtual hosts or to the
 * internal router (see RIAP pseudo-protocol for usage). Components also expose several services:
 * access logging and status setting. <br>
 * <br>
 * From an architectural point of view, here is the REST definition: "A component is an abstract
 * unit of software instructions and internal state that provides a transformation of data via its
 * interface." Roy T. Fielding<br>
 * <br>
 * Components also have useful services associated. They are all enabled by default and are
 * available as properties that can be eventually overridden:
 *
 * <ul>
 *   <li>"logService" to configure access logging.
 *   <li>"statusService" to provide common representations for exception status.
 *   <li>"taskService" to run tasks asynchronously.
 * </ul>
 *
 ...
 *
 * @see <a href= "http://roy.gbiv.com/pubs/dissertation/software_arch.htm#sec_1_2_1">Source
 *     dissertation</a>
...
public class Component extends Restlet {
    ...
    /**
     * Returns the modifiable list of virtual hosts. Note that the order of virtual hosts in this
     * list will be used to check the first one that matches.
     *
    ...
    /**
     * Returns the private internal router where Restlets like Applications can be attached. Those
     * Restlets can be addressed via the {@link org.restlet.data.Protocol#RIAP} (Restlet Internal
     * Access Protocol) client connector. This is used to manage private, internal, and optimized
     * access to local applications.<br>
     * <br>
     * The first use case is the modularization of a large application into modules or layers. This
     * can also be achieved using the {@link Context#getServerDispatcher()} method, but the internal
     * router is easily addressable via a URI scheme and can be fully private to the current
     * Component.<br>
     * <br>
     * The second use case is the composition/mash-up of several representations via the
     * org.restlet.ext.xml.Transformer class, for example. For this you can leverage the XPath's
     * document() function or the XSLT's include and import elements with RIAP URIs.
     *
    ...
    /**
     * Returns the global log service. On the first call, if no log service was defined via the
     * {@link #setLogService(LogService)} method, then a default logger service is created. This
     * service will be enabled by default and has a logger name composed the "org.restlet." Prefix
     * followed by the simple component class name (without packages), followed by the ".LogService"
     * suffix.
     *
    ...
    /**
     * Sets the modifiable list of client connectors. This method clears the current list and adds
    ...
    /**
     * Sets the modifiable list of virtual hosts. Note that the order of virtual hosts in this list
     * will be used to check the first one that matches. This method clears the current list and
    ...
    /**
     * Sets the private internal router where Restlets like Applications can be attached.
     *
    ...
    /**
     * Sets the list of realms. This method clears the current list and adds all entries in the
     * parameter list.
     *
    ...
    /**
     * Sets the modifiable list of server connectors. This method clears the current list and adds
     * all entries in the parameter list.
     *
    ...
    /**
     * Starts the component. First it starts all the connectors (clients then servers), the routers,
     * the services, the realms, and then the component's internal helper. Finally, it calls the
     * start method of the super class.
     *
     * @see #startClients()
     * @see #startServers()
     * @see #startRouters()
    ...
    /**
     * Stops the component. First, it stops the component's internal helper, the realms, the
     * services, the routers and then stops all the connectors (servers then clients). Finally, it
     * calls the stop method of the super class.
     *
     * @see #stopHelper()
     * @see #stopRealms()
     * @see #stopServices()
    ...
    /**
     * Updates the component to take into account changes to the virtual hosts. This method doesn't
     * stop the connectors or the applications or Restlets attached to the virtual hosts. It just
     * updates the internal routes between the virtual hosts and the attached Restlets or
     * applications.
     *
     * @throws Exception
     */
