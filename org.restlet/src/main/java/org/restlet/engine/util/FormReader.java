...
import org.restlet.util.NamedValuesCollector;
...
public class FormReader {
     ...
     *
     * @param representation The web form content.
     * @param decode Indicates if the parameters should be decoded using the given character set.
     * @throws IOException if the stream of the representation could not be opened.
     */
     ...
     */
    public FormReader(
            String parametersString, CharacterSet characterSet, char separator, boolean decode) {
        this.decode = decode;
    ...
    public void addParameters(Series<Parameter> parameters) {
        ...
        if (this.stream != null) {
            ...
            try {
                ...
                Context.getCurrentLogger()
                        .log(Level.WARNING, "Unable to close the form input stream", ioe);
            }
        }
    }
                    ...
                    if (readingName) {
                        if (nextChar == '=') {
                            ...
                            if (!nameBuffer.isEmpty()) {
                                result =
                                        FormUtils.create(
                                                nameBuffer, null, this.decode, this.characterSet);
                            } else if (nextChar == -1) {
    ...
    /**
     * Reads the parameters with the given name. If multiple values are found, a list is returned
     * created.
     *
     * @param name The parameter name to match.
    ...
    public Object readParameter(final String name) throws IOException {
        ...
        NamedValuesCollector collector = new NamedValuesCollector(name);

        if (this.stream != null) {
            Parameter parameter;

            while ((parameter = readNextParameter()) != null) {
                collector.collect(parameter);
            }

            this.stream.close();
        }

        return collector.getCollectedValues().get(name);
    }

    /**
     * Reads the parameters whose name is a key in the given map. If a matching parameter is found,
     * its value is put in the map. If multiple values are found, a list is created and set in the
     * map.
     *
    ...
    public void readParameters(Map<String, Object> parameters) throws IOException {
        if (this.stream != null) {
            ...
            Parameter param;
            while ((param = readNextParameter()) != null) {
