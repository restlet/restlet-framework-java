...
public class ChildClientDispatcher extends TemplateDispatcher {
    ...
    /**
     * Transmits the call to the parent component except if the call is internal as denoted by the
     * {@link Protocol#RIAP} protocol and targets this child application.
     *
    ...
    public int doHandle(Request request, Response response) {
        ...
        if (protocol.equals(Protocol.RIAP)) {
            ...
            if (cr.getRiapAuthorityType() == LocalReference.RIAP_APPLICATION) {
                if ((getChildContext() != null)
                    ...
                    request.getResourceRef()
                            .setBaseRef(request.getResourceRef().getHostIdentifier());
            ...
            if ((getChildContext() != null)
                    && (getChildContext().getChild() instanceof Application application)
                    && !application.getConnectorService().getClientProtocols().contains(protocol)) {
                getLogger()
                        .fine(
