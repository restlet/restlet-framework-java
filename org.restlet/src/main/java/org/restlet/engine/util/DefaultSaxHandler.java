...
/**
 * A Utility class which extends the provided {@link DefaultHandler} and implements the {@link
 * org.w3c.dom.ls.LSResourceResolver} interface. All the methods of this class do nothing besides
...
public class DefaultSaxHandler extends DefaultHandler implements org.w3c.dom.ls.LSResourceResolver {
    ...
    /**
     * A class field set to {@code true} if the JAXP debug property is turned on; {@code false}
     * otherwise. This is used to control the degree of output generated in the logs.
     */
    ...
    /**
     * Set to {@code true} if the current Context's logger is capable of outputting messages at the
     * CONFIG level; {@code false} otherwise.
     */
     ...
     *
     * <p>This implementation <em>always</em> returns a {@code null}.
     *
    ...
    private void log(String level, SAXParseException x) {
        if (loggable) {
            final String msg =
                    "["
                            + level
                            + "] - Unexpected exception while parsing "
                            + "an instance of PUBLIC ["
                            + x.getPublicId()
                            + "], SYSTEM ["
                            + x.getSystemId()
                            + "] - line #"
