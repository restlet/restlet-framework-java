...
public class CallResolver extends Resolver<Object> {
    ...
    public Object resolve(String variableName) {
        ...
        Object result =
                (this.response != null) ? this.response.getAttributes().get(variableName) : null;
        if (result != null) {
            return result;
        }
        ...
        if (this.request != null) {
            ...
            switch (variableName) {
                ...
                case "crs":
                    return (cr != null && cr.getScheme() != null)
                            ? cr.getScheme().getTechnicalName()
                            : null;
                case "d":
                    return DateUtils.format(new Date(), FORMAT_RFC_1123.getFirst());
                case "ecs":
                    return (entity != null && entity.getCharacterSet() != null)
                ...
                case "es":
                    return (entity != null && entity.getSize() != -1)
                            ? Long.toString(entity.getSize())
                            : null;
                case "et":
                    return (entity != null && entity.getTag() != null)
                            ? entity.getTag().getName()
                            : null;
                case "m":
                    return (this.request.getMethod() != null)
                            ? this.request.getMethod().getName()
                            : null;
                case "p":
                    return (this.request.getProtocol() != null)
                            ? this.request.getProtocol().getName()
                            : null;
                default:
                    if (variableName.startsWith("o")) {
                        return getReferenceContent(
                                variableName.substring(1), this.request.getRootRef());
                    } else if (variableName.startsWith("f")) {
                        return getReferenceContent(
                                variableName.substring(1), this.request.getReferrerRef());
                    } else if (variableName.startsWith("h")) {
                        return getReferenceContent(
                                variableName.substring(1), this.request.getHostRef());
                    } else if (variableName.startsWith("r")) {
                        return getReferenceContent(
                                variableName.substring(1), this.request.getResourceRef());
                    }
                    break;
            }
        }
        ...
        if (this.response != null) {
            ...
            switch (variableName) {
                ...
                case "SIP":
                    return (serverInfo.getPort() != -1)
                            ? Integer.toString(serverInfo.getPort())
                            : null;
                default:
                    if (variableName.startsWith("R")) {
                        return getReferenceContent(
                                variableName.substring(1), this.response.getLocationRef());
                    }
            }
        }
     ...
     *
     * @param partName The variable subpart name.
    ...
    private Object getModificationDateAsString(Representation entity) {
        return (entity != null && (entity.getModificationDate() != null))
                ? DateUtils.format(entity.getModificationDate(), FORMAT_RFC_1123.getFirst())
                : null;
    }

    private Object getExpirationDateAsString(Representation entity) {
        return (entity != null && entity.getExpirationDate() != null)
                ? DateUtils.format(entity.getExpirationDate(), FORMAT_RFC_1123.getFirst())
                : null;
    }
