...
public class StrictConneg extends Conneg {
    ...
    /**
     * Scores the annotation descriptor. By default, it assesses the quality of the query parameters
     * with the URI query constraint defined in the annotation value, if any.
     *
    ...
    private float doScoreAnnotation(final MethodAnnotationInfo annotation) {
        ...
        final Form actualParams = getRequest().getResourceRef().getQueryAsForm();
        return scoreQuery(annotation, actualParams);
    }

    private static float scoreQuery(
            final MethodAnnotationInfo annotation, final Form actualParams) {
        final Form constraintParams = new Form(annotation.getQuery());

        final Set<Parameter> matchedParams = new HashSet<>();

        Parameter constraintParam;
        Parameter actualParam;

        boolean allConstraintsMatched = true;
        boolean constraintMatched;
        ...
        for (int i = 0; allConstraintsMatched && (i < constraintParams.size()); i++) {
            ...
            allConstraintsMatched = constraintMatched;
        }
    ...
    public float scoreMediaType(MediaType mediaType) {
        ...
        for (Preference<MediaType> pref : getMediaTypePrefs()) {
            if (pref.getMetadata().includes(mediaType, false)) {
                result = Math.max(pref.getQuality(), result);
            }
        }
    ...
    /**
     * Scores a list of metadata relatively enriched client preferences.
     *
     * @param metadataList The list of metadata to score.
     * @return The score.
     */
    protected <T extends Metadata> float scoreMetadata(
            List<T> metadataList, List<Preference<T>> prefs) {
    ...
    /**
     * Scores a variant relatively to enriched client preferences. The language has a weight of 4,
     * the media type 3, the character set 2, and the encoding 1.
     *
    ...
    public float scoreVariant(Variant variant) {
        ...
        final float languageScore = scoreLanguages(variant.getLanguages());

        if (languageScore != -1.0F) {
            final float mediaTypeScore = scoreMediaType(variant.getMediaType());

            if (mediaTypeScore != -1.0F) {
                final float characterSetScore = scoreCharacterSet(variant.getCharacterSet());

                if (characterSetScore != -1.0F) {
                    final float encodingScore = scoreEncodings(variant.getEncodings());
        ...
        if (Context.getCurrentLogger().isLoggable(Level.FINE)) {
            Context.getCurrentLogger()
                    .fine("Total score of variant \"" + variant + "\"= " + result);
        }

        return result;
    }

    private float scoreVariant(
            final Variant variant,
            final float languageScore,
            final float mediaTypeScore,
            final float characterSetScore,
            final float encodingScore) {
        float result;
        if (variant instanceof VariantInfo variantInfo) {
            final float annotationScore = scoreAnnotation(variantInfo.getAnnotationInfo());
            ...
            result =
                    ((languageScore * 4.0F)
                                    ...
                                    + (annotationScore * 2.0F))
                            + encodingScore / 12.0F;
            // Take into account the affinity with the input entity
            result *= variantInfo.getInputScore();
        } else {
            // Return the weighted average score
            result =
                    ((languageScore * 4.0F)
