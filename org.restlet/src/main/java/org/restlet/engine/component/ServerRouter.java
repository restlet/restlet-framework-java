 ...
 * routers. The host routers then dispatch them to the user applications.
 *
...
public class ServerRouter extends Router {
    ...
    protected void logRoute(org.restlet.routing.Route route) {
        if (getLogger().isLoggable(Level.FINE)) {
            if (route instanceof HostRoute hostRoute) {
                VirtualHost vhost = hostRoute.getVirtualHost();
    ...
    public synchronized void start() throws Exception {
        ...
        final Restlet noHostMatched =
                new Restlet(getComponent().getContext().createChildContext()) {
                    @Override
