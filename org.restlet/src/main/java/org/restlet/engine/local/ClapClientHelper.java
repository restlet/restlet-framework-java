...
/**
 * Connector to the resources accessed via class loaders. Note that if you use the class authority
 * for your CLAP URIs, you can provide a custom classloader instead of the one of the connector. For
 * this, your requests need to have a "org.restlet.clap.classLoader" attribute set with the instance
 * of your classloader and use the {@link LocalReference#CLAP_CLASS} authority.
 *
...
public class ClapClientHelper extends LocalClientHelper {
    ...
    protected void handleClassLoader(Request request, Response response, ClassLoader classLoader) {
        ...
        try {
            ...
            if (url.getProtocol().equals("jar") && inputStream.available() == 0) {
                response.setStatus(Status.CLIENT_ERROR_NOT_FOUND);
                return;
            }

            Representation output =
                    new InputRepresentation(inputStream, metadataService.getDefaultMediaType());
            ...
            if (timeToLive == 0) {
                ...
                output.setExpirationDate(
                        new Date(System.currentTimeMillis() + (1000L * timeToLive)));
            }
    ...
    protected void handleLocal(Request request, Response response, String decodedPath) {
        ...
        if (scheme.equalsIgnoreCase(Protocol.CLAP.getSchemeName())) {
            ...
            if ((cr.getClapAuthorityType() == LocalReference.CLAP_CLASS)
                ...
                Object classLoaderAttribute =
                        request.getAttributes().get("org.restlet.clap.classLoader");
