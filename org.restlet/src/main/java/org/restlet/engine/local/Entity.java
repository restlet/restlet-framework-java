...
public abstract class Entity {
    ...
    /**
     * Return the base name that is to say the longest part of a given name without known extensions
    ...
    public static Collection<String> getExtensions(
            Variant variant, MetadataService metadataService) {
        final Set<String> result = new TreeSet<>();
    ...
    public static void updateMetadata(
        ...
        for (int j = 1; j < tokens.length; j++) {
            ...
            updateMetadata(variant, current);
            ...
            if (dashIndex != -1) {
                ...
                if (current instanceof Language language) {
                    variant.getLanguages().add(language);
                }
            }
        }

        if (applyDefault) {
            applyDefaultLanguageIfNecessary(variant, metadataService);
            applyDefaultMediaTypeIfNecessary(variant, metadataService);
            applyDefaultEncodingIfNecessary(variant, metadataService);
            applyDefaultCharacterSetIfNecessary(variant, metadataService);
        }
    }

    private static void updateMetadata(final Variant variant, final Metadata metadata) {
        if (metadata == null) {
            return;
        }
        ...
        switch (metadata) {
            case MediaType mediaType -> variant.setMediaType(mediaType);
            case CharacterSet characterSet -> variant.setCharacterSet(characterSet);
            case Encoding encoding -> {
                // Do we need to add this metadata?
                boolean found = false;
                for (int i = 0; !found && i < variant.getEncodings().size(); i++) {
                    found = metadata.includes(variant.getEncodings().get(i));
                }
                if (!found) {
            ...
            case Language language -> {
                ...
                for (int i = 0; !found && i < variant.getLanguages().size(); i++) {
                    found = metadata.includes(variant.getLanguages().get(i));
                }
                if (!found) {
                    variant.getLanguages().add(language);
                }
            }
            default -> throw new IllegalStateException("Unexpected metadata: " + metadata);
        }
    }

    private static void applyDefaultCharacterSetIfNecessary(
            final Variant variant, final MetadataService metadataService) {
    ...
    private static void applyDefaultEncodingIfNecessary(
            final Variant variant, final MetadataService metadataService) {
    ...
    private static void applyDefaultMediaTypeIfNecessary(
            final Variant variant, final MetadataService metadataService) {
    ...
    private static void applyDefaultLanguageIfNecessary(
            final Variant variant, final MetadataService metadataService) {
     ...
     */
    protected Entity(MetadataService metadataService) {
     ...
     *
     * @return True if the entity does exist.
     */
    public abstract boolean exists();

    /**
     * Return the base name of this entity, meaning the longest part of the name without known
