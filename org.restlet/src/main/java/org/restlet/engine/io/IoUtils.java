...
public class IoUtils {
    ...
    /**
     * The size to use when instantiating buffered items such as instances of the {@link
     * BufferedReader} class. It looks for the System property "org.restlet.engine.io.bufferSize"
     * and if not defined, uses the "8192" default value.
     */
    ...
    /**
     * The number of milliseconds after which IO operation will time out. It looks for the System
     * property "org.restlet.engine.io.timeoutMs" and if not defined, uses the "60000" default
     * value.
     */
    public static final int TIMEOUT_MS = getProperty("org.restlet.engine.io.timeoutMs", 60000);

    /**
     * Copies an input stream to an output stream. When the reading is done, the input stream is
     * closed.
     *
     ...
     * @throws IOException
     */
    public static void copy(InputStream inputStream, java.io.OutputStream outputStream)
            throws IOException {
        if (inputStream != null) {
    ...
    /**
     * Copies an input stream to a random access file. When the reading is done, the input stream is
     * closed.
     *
     * @param inputStream The input stream.
     ...
     * @throws IOException
     */
    public static void copy(InputStream inputStream, java.io.RandomAccessFile randomAccessFile)
            throws IOException {
        int bytesRead;
    ...
    /**
     * Deletes an individual file or a directory. A recursive deletion can be forced as well. Under
     * Windows operating systems, the garbage collector will be invoked once before attempting to
     * delete to prevent locking issues.
     *
     * @param file The individual file or directory to delete.
     * @param recursive Indicates if a directory with content should be deleted recursively as well.
     * @return True if the deletion was successful or if the file or directory didn't exist.
     */
    ...
    /**
     * Deletes an individual file or a directory. A recursive deletion can be forced as well. The
     * garbage collector can be run once before attempting to delete, to workaround lock issues
    ...
    /**
     * Returns the size effectively available. This returns the same value as {@link
     * Representation#getSize()} if no range is defined, otherwise it returns the size of the range
     * using {@link Range#getSize()}.
     *
    ...
    public static long getAvailableSize(Representation representation) {
        ...
        if (range == null || !isBytesRange(range)) {
            ...
            if (representation.hasKnownSize()) {
                return Math.min(range.getIndex() + range.getSize(), representation.getSize())
                        - range.getIndex();
            } else {
     ...
     * @param characterSet The character set. Maybe null.
    ...
    /**
     * Returns a reader from a writer representation. Internally, it uses a writer thread and a pipe
     * stream.
     *
    ...
    public static Reader getReader(
        ...
        Runnable task =
                new org.restlet.engine.util.ContextualRunnable() {
                    public void run() {
                        try {
                            ...
                            try {
                                ...
                                Context.getCurrentLogger()
                                        .log(Level.WARNING, "Error while closing the pipe.", ioe2);
                            }
                        }
                    }
                };
     ...
     * write(OutputStream) method. Internally, it uses a writer thread and a pipe stream.
     *
     * @param representation the representation to get the {@link java.io.OutputStream} from.
    ...
    /**
     * Returns a writer to the given output stream, using the given character set for encoding to
     * bytes.
     *
    ...
    public static Writer getWriter(OutputStream outputStream, CharacterSet characterSet) {
        // Use the default HTTP character set if necessary
        final Charset charset =
                Objects.requireNonNullElse(characterSet, CharacterSet.ISO_8859_1).toCharset();
     ...
     * As this method uses the InputStreamReader class, the default character set is used for
    ...
    /**
     * Converts an input stream to a string using the specified character set for decoding the input
     * stream. Once read, the input stream is closed.
     *
     * @see InputStreamReader
     * @param inputStream The input stream.
        ...
        if (inputStream != null) {
            try {
                if (characterSet != null) {
                    result =
                            IoUtils.toString(
                                    new InputStreamReader(inputStream, characterSet.getName()));
                } else {
    ...
    public static String toString(Reader reader) {
        ...
        if (reader != null) {
            try {
                ...
                BufferedReader br =
                        (reader instanceof BufferedReader bufferedReader)
                                ? bufferedReader
