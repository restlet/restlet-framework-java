...
/**
 * SSL server socket factory that wraps the default one to do extra initialization. Configures the
 * cipher suites and the SSL certificate request.
 *
...
public class WrapperSslServerSocketFactory extends SSLServerSocketFactory {
    ...
    public WrapperSslServerSocketFactory(
            DefaultSslContextFactory contextFactory, SSLServerSocketFactory wrappedSocketFactory) {
    ...
    public ServerSocket createServerSocket(int port) throws IOException {
        SSLServerSocket result =
                (SSLServerSocket) getWrappedSocketFactory().createServerSocket(port);
        return initSslServerSocket(result);
    }
    ...
    public ServerSocket createServerSocket(int port, int backLog) throws IOException {
        SSLServerSocket result =
                (SSLServerSocket) getWrappedSocketFactory().createServerSocket(port, backLog);
        return initSslServerSocket(result);
    }
    ...
    public ServerSocket createServerSocket(int port, int backLog, InetAddress ifAddress)
        ...
        SSLServerSocket result =
                (SSLServerSocket)
                        getWrappedSocketFactory().createServerSocket(port, backLog, ifAddress);
        return initSslServerSocket(result);
    }
    ...
    /**
     * Initializes the SSL server socket. Configures the certificate request (need or want) and the
    ...
    protected SSLServerSocket initSslServerSocket(SSLServerSocket sslServerSocket) {
        ...
        if ((getContextFactory().getEnabledCipherSuites() != null)
            ...
            sslServerSocket.setEnabledCipherSuites(
                    getContextFactory()
                            .getSelectedCipherSuites(sslServerSocket.getSupportedCipherSuites()));
        }
        ...
        if ((getContextFactory().getEnabledProtocols() != null)
            ...
            sslServerSocket.setEnabledProtocols(
                    getContextFactory()
                            .getSelectedSslProtocols(sslServerSocket.getSupportedProtocols()));
        }
