...
public class HttpRequest extends Request {
        ...
        if (httpCall.getRequestUri() != null) {
            ...
            if (getResourceRef().isRelative()) {
                ...
                if (!httpCall.getRequestUri().startsWith("/")) {
                    setResourceRef(
                            new Reference(
                                    getHostRef().toString() + "/" + httpCall.getRequestUri()));
                } else {
                    setResourceRef(
                            new Reference(getHostRef().toString() + httpCall.getRequestUri()));
                }
            }

            setOriginalRef(
                    ReferenceUtils.getOriginalRef(getResourceRef(), httpCall.getRequestHeaders()));
        }

        // Set the request date
        String dateHeader =
                httpCall.getRequestHeaders().getFirstValue(HeaderConstants.HEADER_DATE, true);
        Date date = null;
        if (dateHeader != null) {
        ...
        if (!accessControlRequestHeadersAdded) {
            for (String header :
                    getHttpCall()
                            .getRequestHeaders()
                            .getValuesArray(
                                    HeaderConstants.HEADER_ACCESS_CONTROL_REQUEST_HEADERS, true)) {
    ...
    public List<CacheDirective> getCacheDirectives() {
        ...
        if (!cacheDirectivesAdded) {
            for (Header header :
                    getHttpCall()
                            .getRequestHeaders()
                            .subList(HeaderConstants.HEADER_CACHE_CONTROL)) {
    ...
    public ChallengeResponse getChallengeResponse() {
        ...
        if (!this.securityAdded) {
            ...
            String authorization =
                    getHttpCall()
                            .getRequestHeaders()
                            .getValues(HeaderConstants.HEADER_AUTHORIZATION);

            // Set the challenge response
            result =
                    AuthenticatorUtils.parseResponse(
                            this, authorization, getHttpCall().getRequestHeaders());
    ...
    public ClientInfo getClientInfo() {
        ...
        extractPreferences(result);

        // Set other properties
        result.setAgent(
                getHttpCall().getRequestHeaders().getValues(HeaderConstants.HEADER_USER_AGENT));
        result.setFrom(
                getHttpCall().getRequestHeaders().getFirstValue(HeaderConstants.HEADER_FROM, true));
    ...
    private void extractPreferences(final ClientInfo result) {
        ...
        try {
            String acceptMediaType =
                    getHttpCall().getRequestHeaders().getValues(HeaderConstants.HEADER_ACCEPT);
        ...
        try {
            String acceptPatch =
                    getHttpCall()
                            .getRequestHeaders()
                            .getValues(HeaderConstants.HEADER_ACCEPT_PATCH);
        ...
        try {
            String expect =
                    getHttpCall().getRequestHeaders().getValues(HeaderConstants.HEADER_EXPECT);
    ...
    public Conditions getConditions() {
        ...
        extractIfMatch(result);
        extractIfNoneMatch(result);
        extractIfRangeCondition(result);
    ...
    private void extractIfNoneMatch(final Conditions result) {
        // Set the If-None-Match tags
        String ifNoneMatchHeader =
                getHttpCall().getRequestHeaders().getValues(HEADER_IF_NONE_MATCH);
        if (ifNoneMatchHeader != null) {
            try {
                ...
                this.context
                        ...
                        .log(
                                Level.INFO,
                                "Unable to process the if-none-match header: {0}",
                                ifNoneMatchHeader);
            }
        }
    }

    private void extractIfMatch(final Conditions result) {
        ...
        if (ifMatchHeader != null) {
            try {
                ...
                this.context
                        ...
                        .log(
                                Level.INFO,
                                "Unable to process the if-match header: {0}",
                                ifMatchHeader);
            }
        }
    }

    private void extractIfRangeCondition(final Conditions result) {
        String ifRangeHeader =
                getHttpCall().getRequestHeaders().getFirstValue(HEADER_IF_RANGE, true);
    ...
    public Series<Cookie> getCookies() {
        ...
        if (!this.cookiesAdded) {
            String cookieValues =
                    getHttpCall().getRequestHeaders().getValues(HeaderConstants.HEADER_COOKIE);
    ...
    public List<Range> getRanges() {
        ...
        if (!this.rangesAdded) {
            // Extract the header value
            final String ranges =
                    getHttpCall().getRequestHeaders().getValues(HeaderConstants.HEADER_RANGE);
    ...
    public Reference getReferrerRef() {
        if (!this.referrerAdded) {
            final String referrerValue =
                    getHttpCall().getRequestHeaders().getValues(HeaderConstants.HEADER_REFERRER);
