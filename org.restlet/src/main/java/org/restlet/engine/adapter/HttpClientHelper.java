...
/**
 * Base HTTP client connector. Here is the list of parameters that are supported. They should be set
...
public abstract class HttpClientHelper extends ClientHelper {
     ...
     */
    protected HttpClientHelper(Client client) {
        super(client);
        this.adapter = null;
    }
    ...
    public ClientAdapter getAdapter() throws Exception {
        if (this.adapter == null) {
            String adapterClass =
                    getHelpedParameters()
                            .getFirstValue("adapter", "org.restlet.engine.adapter.ClientAdapter");
            this.adapter =
                    (ClientAdapter)
    ...
    public int getSocketConnectTimeoutMs() {
        return Integer.parseInt(
                getHelpedParameters().getFirstValue("socketConnectTimeoutMs", "15000"));
    }
