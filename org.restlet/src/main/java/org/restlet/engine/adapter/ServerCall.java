...
public abstract class ServerCall extends Call {
     ...
     */
    protected ServerCall(Server server) {
        this(
                (server == null) ? null : server.getAddress(),
     ...
     */
    protected ServerCall(String serverAddress, int serverPort) {
    ...
    /**
     * Flushes the buffers onto the network so that for example you can force headers to be written
     * before the entity is becoming available.
     *
     * @throws IOException
     */
    ...
    public Representation getRequestEntity() {
        ...
        if (((contentLength != Representation.UNKNOWN_SIZE) && (contentLength != 0))
            ...
            InputStream requestStream = getInputStream(contentLength, connectionClosed);
        ...
        extractRequestHeaders(result);

        return result;
    }

    private InputStream getInputStream(final long contentLength, final boolean connectionClosed) {
        ...
        if (connectionClosed) {
            ...
            // the end of the connection can let us know at this point
    ...
    private void extractRequestHeaders(final Representation result) {
            ...
            if (header.getName().equalsIgnoreCase(HeaderConstants.HEADER_CONTENT_ENCODING)) {
                ...
                result.setDigest(
                        new Digest(
                                ...
                                Base64.getDecoder().decode(header.getValue())));
            } else if (header.getName()
                    .equalsIgnoreCase(HeaderConstants.HEADER_CONTENT_DISPOSITION)) {
                try {
     ...
     *
     * @return The SSL session ID, in hexadecimal encoding, if available and accessible.
     */
    ...
    /**
     * Returns the SSL session ID, as a byte array, if available and accessible in that format (to
     * be used by getSslSessionId).
     *
     * @return The SSL session ID, as a byte array, if available and accessible in that format.
     */
     ...
     * send them over the network. The default implementation only writes the response entity on the
     * response stream or channel. Subclasses will probably also copy the response headers and
     * status.
     *
    ...
    /**
     * Effectively writes the response body. The entity to write is guaranteed to be non-null.
     * Attempts to write the entity on the response channel or response stream by default.
     *
