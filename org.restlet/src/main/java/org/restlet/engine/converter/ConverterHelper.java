...
public abstract class ConverterHelper extends Helper {
     ...
     * @throws IOException
     */
    public List<VariantInfo> addVariants(
        ...
        if (helperVariants != null) {
            ...
            for (VariantInfo helperVariant : helperVariants) {
                if (targetVariant == null) {
                    ...
                    // Detected a more generic variant, but still consider the conversion is
                    // possible to the target variant.
                    variants = addVariant(variants, new VariantInfo(targetVariant.getMediaType()));
                } else if (targetVariant.includes(helperVariant)) {
                    // Detected a more specific variant, but still consider the conversion is
                    // possible to the target variant.
                    variants = addVariant(variants, helperVariant);
                }
            }
    ...
    /**
     * Returns the list of variants that can be converted from a given object class. The preferred
     * variant should be set in the first position.
     *
    ...
    /**
     * Returns the list of variants that can be converted from a given object class by a specific
     * converter helper.
     *
     * @param sourceClass The source class.
     * @param targetVariant The expected representation metadata.
     ...
     */
    public abstract <T> T toObject(Representation source, Class<T> target, Resource resource)
            throws IOException;
    ...
    public abstract Representation toRepresentation(
            Object source, Variant target, Resource resource) throws IOException;

    /**
     * Updates the preferences of the given {@link ClientInfo} object with conversion capabilities
    ...
    public void updatePreferences(
            List<Preference<MediaType>> preferences, MediaType mediaType, float score) {
        boolean found = false;
        ...
        if (!found) {
            preferences.add(new Preference<>(mediaType, score));
        }
    }
}
