 ...
 *
 * @param <V> The header value target type. There can be multiple values for a single header.
 * @author Jerome Louvel
 */
public class HeaderReader<V> {

    /**
     * Creates a new NamedValue with a null value.
     *
     * @param name The name.
     * @param resultClass The NamedValue class to return.
     * @return The new NamedValue.
     */
    private static <N extends NamedValue<String>> N createNamedValue(
            Class<N> resultClass, String name) {
     ...
     */
    private static <N extends NamedValue<String>> N createNamedValue(
            Class<N> resultClass, String name, String value) {
        try {
    ...
    public static Header readHeader(CharSequence header) throws IOException {
        ...
        if (isCarriageReturn(next)) { // Detect the end of headers
    ...
    public static Header readHeader(InputStream is, StringBuilder sb) throws IOException {
        ...
        while (isSpace(next)) { // Skip any separator space between colon and header value
            next = is.read();
        }
    ...
    /**
     * Indicates if the value can be added to the list. Useful to prevent the addition of {@link
     * Encoding#IDENTITY} constants for example. By default, it returns true for non-null values.
     *
    ...
    /**
     * Marks the current position in this reader. A later call to the <code>reset</code> method
     * repositions this reader at the last marked position.
     */
    public void mark() {
        mark = index;
    }
    ...
    public String readComment() throws IOException {
        ...
        // The first character must be a parenthesis
        if (next == '(') {
     ...
     * @throws IOException
     */
    public <N extends NamedValue<String>> N readNamedValue(Class<N> resultClass)
            throws IOException {
        N result = null;
     ...
     *
     * @see <a href= "http://www.w3.org/Protocols/rfc2616/rfc2616-sec2.html#sec2">HTTP parsing
     *     rule</a>
    ...
    /**
     * Read the next value. There can be multiple values for a single header. Returns null by
     * default.
     *
    ...
    public List<V> readValues() {
        List<V> result = new CopyOnWriteArrayList<>();
        addValues(result);
        return result;
    }
    ...
    /**
     * Skips the next parameter separator (semicolon) including leading and trailing spaces.
     *
    ...
    public boolean skipValueSeparator() {
        ...
        // Check if the next character is a value separator
        if (isComma(read())) {
            result = true;
