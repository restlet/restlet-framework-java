...
public class PreferenceReader<T extends Metadata> extends HeaderReader<Preference<T>> {
        ...
        if (acceptCharsetHeader != null) {
            ...
            if (acceptCharsetHeader.isEmpty()) {
                clientInfo
                        ...
                        .add(new Preference<>(CharacterSet.ISO_8859_1));
            } else {
                PreferenceReader pr =
                        new PreferenceReader(
    ...
    public static void addEncodings(String acceptEncodingHeader, ClientInfo clientInfo) {
        if (acceptEncodingHeader != null) {
            PreferenceReader pr =
                    new PreferenceReader(PreferenceReader.TYPE_ENCODING, acceptEncodingHeader);
    ...
    public static void addLanguages(String acceptLanguageHeader, ClientInfo clientInfo) {
        if (acceptLanguageHeader != null) {
            PreferenceReader pr =
                    new PreferenceReader(PreferenceReader.TYPE_LANGUAGE, acceptLanguageHeader);
    ...
    public static void addMediaTypes(String acceptMediaTypeHeader, ClientInfo clientInfo) {
        if (acceptMediaTypeHeader != null) {
            PreferenceReader pr =
                    new PreferenceReader(PreferenceReader.TYPE_MEDIA_TYPE, acceptMediaTypeHeader);
    ...
    public static void addPatches(String acceptPatchHeader, ClientInfo clientInfo) {
        if (acceptPatchHeader != null) {
            PreferenceReader pr =
                    new PreferenceReader(PreferenceReader.TYPE_PATCH, acceptPatchHeader);
    ...
    public static float readQuality(String quality) {
        try {
            float result = Float.parseFloat(quality);
    ...
    protected Preference<T> createPreference(CharSequence metadata, Series<Parameter> parameters) {
        ...
        if (parameters == null) {
            ...
            switch (this.type) {
                ...
                case TYPE_MEDIA_TYPE, TYPE_PATCH:
    ...
    public Preference<T> readValue() throws IOException {
        ...
        while (result == null) {
            ...
            if (readingMetadata) {
                ...
                } else if (isSpace(next) && paramNameBuffer.isEmpty()) {
                    // Ignore white spaces
                ...
                if ((next == -1) || isComma(next) || isSpace(next)) {
                    ...
                    readingParamName = true;
                    readingParamValue = false;
                } else if ((next == '"') && paramValueBuffer.isEmpty()) {
