...
public class HeaderUtils {
    ...
    /** Standard set of headers that cannot be modified. */
    ...
    private static final Set<String> UNSUPPORTED_STANDARD_HEADERS =
            Collections.unmodifiableSet(
                    new CaseInsensitiveHashSet(
                            Arrays.asList(
                                    HEADER_PRAGMA,
    ...
    /**
     * Map of handlers that update a response for a specific header. More efficient than an if/else
     * lookup.
     */
    private static final Map<String, HeaderHandler> RESPONSE_HEADER_HANDLERS;

    /**
     * Map of handlers that update a representation for a specific header. More efficient than an
     * if/else lookup.
     */
    private static final Map<String, RepresentationHeaderHandler> REPRESENTATION_HEADER_HANDLERS;

    static {
        RESPONSE_HEADER_HANDLERS =
                Map.ofEntries(
                        Map.entry(
                                HEADER_LOCATION.toLowerCase(),
                                (h, r, all) -> r.setLocationRef(h.getValue())),
                        Map.entry(
                                HEADER_AGE.toLowerCase(),
                                (h, r, all) -> {
                                    try {
                                        r.setAge(Integer.parseInt(h.getValue()));
                        ...
                        Map.entry(
                                HEADER_DATE.toLowerCase(),
                                (h, r, all) -> {
                                    Date date = DateUtils.parse(h.getValue());
                                    r.setDate(date != null ? date : new Date());
                                }),
                        Map.entry(
                                HEADER_RETRY_AFTER.toLowerCase(),
                                (h, r, all) -> {
                                    Date retryAfter = DateUtils.parse(h.getValue());

                                    if (retryAfter == null) {
                                        // The date might be expressed as a number of seconds
                                        try {
                                            int retryAfterSecs =
                                                    (int) Double.parseDouble(h.getValue());
                                ...
                                (h, r, all) -> {
                                    try {
                                        CookieSettingReader cr =
                                                new CookieSettingReader(h.getValue());
                                        r.getCookieSettings().add(cr.readValue());
                                    } catch (Exception e) {
                                        Context.getCurrentLogger()
                                                .log(
                                                        Level.WARNING,
                        ...
                        Map.entry(
                                HEADER_SET_COOKIE2.toLowerCase(),
                                (h, r, all) -> {
                                    try {
                                        CookieSettingReader cr =
                                                new CookieSettingReader(h.getValue());
                        ...
                        Map.entry(
                                ...
                                (h, r, all) ->
                                        r.getChallengeRequests()
                                                .addAll(
                                                        AuthenticatorUtils.parseRequest(
                                                                r, h.getValue(), all))),
                        Map.entry(
                                HEADER_PROXY_AUTHENTICATE.toLowerCase(),
                                (h, r, all) ->
                                        r.getProxyChallengeRequests()
                                                .addAll(
                                                        AuthenticatorUtils.parseRequest(
                                                                r, h.getValue(), all))),
                        Map.entry(
                                HEADER_AUTHENTICATION_INFO.toLowerCase(),
                                (h, r, all) ->
                                        r.setAuthenticationInfo(
                                                AuthenticatorUtils.parseAuthenticationInfo(
                                                        h.getValue()))),
                        Map.entry(
                                HEADER_SERVER.toLowerCase(),
                                (h, r, all) -> r.getServerInfo().setAgent(h.getValue())),
                        Map.entry(
                                HEADER_ALLOW.toLowerCase(),
                                (h, r, all) -> MethodReader.addValues(h, r.getAllowedMethods())),
                        Map.entry(
                                HEADER_VARY.toLowerCase(),
                                (h, r, all) -> DimensionReader.addValues(h, r.getDimensions())),
                        Map.entry(
                                HEADER_VIA.toLowerCase(),
                                (h, r, all) ->
                                        RecipientInfoReader.addValues(h, r.getRecipientsInfo())),
                        Map.entry(
                                HEADER_WARNING.toLowerCase(),
                                (h, r, all) -> WarningReader.addValues(h, r.getWarnings())),
                        Map.entry(
                                HEADER_CACHE_CONTROL.toLowerCase(),
                                (h, r, all) ->
                                        CacheDirectiveReader.addValues(h, r.getCacheDirectives())),
                        Map.entry(
                                HEADER_ACCEPT_RANGES.toLowerCase(),
                                (h, r, all) -> {
                                    TokenReader tr = new TokenReader(h.getValue());
                                    r.getServerInfo()
                                            .setAcceptingRanges(
                                                    tr.readValues().contains(RANGE_BYTES_UNIT));
                                }),
                        Map.entry(
                                HEADER_ACCESS_CONTROL_ALLOW_CREDENTIALS.toLowerCase(),
                                (h, r, all) ->
                                        r.setAccessControlAllowCredentials(
                                                parseBoolean(h.getValue()))),
                        Map.entry(
                                HEADER_ACCESS_CONTROL_ALLOW_HEADERS.toLowerCase(),
                                (h, r, all) ->
                                        StringReader.addValues(
                                                h, r.getAccessControlAllowHeaders())),
                        Map.entry(
                                HEADER_ACCESS_CONTROL_ALLOW_ORIGIN.toLowerCase(),
                                (h, r, all) -> r.setAccessControlAllowOrigin(h.getValue())),
                        Map.entry(
                                HEADER_ACCESS_CONTROL_ALLOW_METHODS.toLowerCase(),
                                (h, r, all) ->
                                        MethodReader.addValues(
                                                h, r.getAccessControlAllowMethods())),
                        Map.entry(
                                HEADER_ACCESS_CONTROL_MAX_AGE.toLowerCase(),
                                (h, r, all) ->
                                        r.setAccessControlMaxAge(Integer.parseInt(h.getValue()))));

        REPRESENTATION_HEADER_HANDLERS =
                Map.ofEntries(
                        Map.entry(
                                HEADER_CONTENT_TYPE.toLowerCase(),
                                (h, r) -> {
                                    ContentType contentType = new ContentType(h.getValue());
                                    r.setMediaType(contentType.getMediaType());

                                    if ((r.getCharacterSet() == null)
                                            || (contentType.getCharacterSet() != null)) {
                                        r.setCharacterSet(contentType.getCharacterSet());
                                    }
                                }),
                        Map.entry(HEADER_CONTENT_LENGTH.toLowerCase(), (h, r) -> {}),
                        Map.entry(
                                HEADER_EXPIRES.toLowerCase(),
                                (h, r) ->
                                        r.setExpirationDate(
                                                HeaderReader.readDate(h.getValue(), false))),
                        Map.entry(
                                HEADER_CONTENT_ENCODING.toLowerCase(),
                                (h, r) ->
                                        new EncodingReader(h.getValue())
                                                .addValues(r.getEncodings())),
                        Map.entry(
                                HEADER_CONTENT_LANGUAGE.toLowerCase(),
                                (h, r) ->
                                        new LanguageReader(h.getValue())
                                                .addValues(r.getLanguages())),
                        Map.entry(
                                HEADER_LAST_MODIFIED.toLowerCase(),
                                (h, r) ->
                                        r.setModificationDate(
                                                HeaderReader.readDate(h.getValue(), false))),
                        Map.entry(
                                HEADER_ETAG.toLowerCase(),
                                (h, r) -> r.setTag(Tag.parse(h.getValue()))),
                        Map.entry(
                                HEADER_CONTENT_LOCATION.toLowerCase(),
                                (h, r) -> r.setLocationRef(h.getValue())),
                        Map.entry(
                                HEADER_CONTENT_DISPOSITION.toLowerCase(),
                                (h, r) -> {
                                    try {
                                        r.setDisposition(
                                                new DispositionReader(h.getValue()).readValue());
                        ...
                        Map.entry(
                                HEADER_CONTENT_RANGE.toLowerCase(),
                                (h, r) -> RangeReader.update(h.getValue(), r)),
                        Map.entry(
                                HEADER_CONTENT_MD5.toLowerCase(),
                                (h, r) -> {
                                    ...
                                    String base64hash = h.getValue();
                                    if (base64hash.length() == 22) {
                                        base64hash += "==";
                                    }
                                    r.setDigest(
                                            new org.restlet.data.Digest(
    ...
    public static void addEntityHeaders(Representation entity, Series<Header> headers) {
        addEntityContentLengthHeader(entity, headers);

        if (entity != null) {
            addEntityContentHeaders(entity, headers);
            addEntityMetadataHeaders(entity, headers);
        }
    }

    private static void addEntityContentHeaders(
            final Representation entity, final Series<Header> headers) {
        ...
        if (entity.getDigest() != null && ALGORITHM_MD5.equals(entity.getDigest().getAlgorithm())) {
            addHeader(
                    HEADER_CONTENT_MD5,
                    new String(java.util.Base64.getEncoder().encode(entity.getDigest().getValue())),
                    headers);
        }
        ...
        if (entity.getRange() != null) {
            ...
            if (isBytesRange(range)) {
                addHeader(
                        HEADER_CONTENT_RANGE, RangeWriter.write(range, entity.getSize()), headers);
            } else {
                addHeader(
                        HEADER_CONTENT_RANGE,
    ...
    private static void addEntityMetadataHeaders(
            final Representation entity, final Series<Header> headers) {
        ...
        if (entity.getModificationDate() != null) {
            addHeader(
                    HEADER_LAST_MODIFIED, DateWriter.write(entity.getModificationDate()), headers);
        }
    ...
    private static void addEntityContentLengthHeader(
            final Representation entity, final Series<Header> headers) {
     ...
     */
    public static void addUserExtensionHeaders(
            Series<Header> existingHeaders, Series<Header> additionalHeaders) {
            ...
            try {
                ...
                Context.getCurrentLogger()
                        .log(
                                Level.WARNING,
                                exception,
                                () -> "Unable to format the " + headerName + " header");
            }
        }
    }

    /**
     * Adds the entity headers based on the {@link Representation} to the {@link Series} when a 304
     * (Not Modified) status is returned.
     *
     * @param entity The source entity {@link Representation}.
     * @param headers The target headers {@link Series}.
     */
    ...
    public static void addRequestHeaders(Request request, Series<Header> headers) {
        ...
        addRequestPreferencesHeaders(clientInfo, headers);

        if (!clientInfo.getExpectations().isEmpty()) {
            addHeader(
                    HEADER_EXPECT, ExpectationWriter.write(clientInfo.getExpectations()), headers);
        }
        ...
        addRequestHostHeader(request, headers);

        addRequestConditionsHeaders(request.getConditions(), headers);
        ...
        // CORS headers
        addRequestCorsHeaders(request, headers);
        ...
        // -------------------------------------
        addUserExtensionHeaders(headers, request.getHeaders());
        ...
        addRequestAuthorizationHeaders(request, headers);
    }

    private static void addRequestAuthorizationHeaders(
            final Request request, final Series<Header> headers) {
        ChallengeResponse challengeResponse = request.getChallengeResponse();

        if (challengeResponse != null) {
            String authHeader =
                    AuthenticatorUtils.formatResponse(challengeResponse, request, headers);
        ...
        if (proxyChallengeResponse != null) {
            String authHeader =
                    AuthenticatorUtils.formatResponse(proxyChallengeResponse, request, headers);
    ...
    private static void addRequestConditionsHeaders(
            final Conditions conditions, final Series<Header> headers) {
        ...
        if (conditions.getModifiedSince() != null) {
            addHeader(
                    HEADER_IF_MODIFIED_SINCE,
                    DateWriter.write(conditions.getModifiedSince()),
                    headers);
        }
    ...
    private static void addRequestCorsHeaders(final Request request, final Series<Header> headers) {
    ...
    private static void addRequestHostHeader(final Request request, final Series<Header> headers) {
        ...
        Reference hostRef =
                (request.getResourceRef().getBaseRef() != null)
    ...
    private static void addRequestPreferencesHeaders(
            final ClientInfo clientInfo, final Series<Header> headers) {
        ...
        if (!clientInfo.getAcceptedCharacterSets().isEmpty()) {
            addHeader(
                    HEADER_ACCEPT_CHARSET,
                    PreferenceWriter.write(clientInfo.getAcceptedCharacterSets()),
                    headers);
        }
    ...
    public static void addResponseHeaders(Response response, Series<Header> headers) {
        addResponseServerMetadataHeaders(response, headers);
        ...
        if (response.getLocationRef() != null) {
            ...
            addHeader(
                    HEADER_LOCATION, response.getLocationRef().getTargetRef().toString(), headers);
        }

        addResponseProxyHeaders(response, headers);
        ...
        // to partially and badly support this header (cf. issue 261).
        if (!((response.getRequest().getClientInfo().getAgent() != null)
                && response.getRequest().getClientInfo().getAgent().contains("MSIE"))) {
        ...
        addResponseSecurityHeaders(response, headers);

        // CORS headers
        addResponseCorsHeaders(response, headers);

        // ----------------------------------
        // 3) Add supported extension headers
        // ----------------------------------

        addExtensionHeaders(response, headers);

        // -------------------------------------
        // 4) Add user-defined extension headers
        // -------------------------------------

        addUserExtensionHeaders(headers, response.getHeaders());
    }

    private static void addResponseServerMetadataHeaders(
            final Response response, final Series<Header> headers) {
    ...
    private static void addResponseProxyHeaders(
            final Response response, final Series<Header> headers) {
        ...
        if (response.getProxyChallengeRequests() != null) {
            for (ChallengeRequest challengeRequest : response.getProxyChallengeRequests()) {
                addHeader(
                        HEADER_PROXY_AUTHENTICATE,
                        AuthenticatorUtils.formatRequest(challengeRequest, response, headers),
                        headers);
            }
        }
    }

    private static void addResponseSecurityHeaders(
            final Response response, final Series<Header> headers) {
    ...
    private static void addExtensionHeaders(final Response response, final Series<Header> headers) {
        // Add the Authentication-Info header
        if (response.getAuthenticationInfo() != null) {
            addHeader(
                    HEADER_AUTHENTICATION_INFO,
                    AuthenticatorUtils.formatAuthenticationInfo(response.getAuthenticationInfo()),
                    headers);
        }

        // Cookie-settings should be written in a single header, but Web
        // browsers do not seem to support it.
    ...
    private static void addResponseCorsHeaders(
            final Response response, final Series<Header> headers) {
    ...
    /**
     * Remove the headers that are mapped to the framework's API from the given message's list of
     * headers.
     *
    ...
    public static void keepExtensionHeadersOnly(Message message) {
        ...
        Series<Header> extensionHeaders = new Series<>(Header.class);
    ...
    public static void copyResponseTransportHeaders(Series<Header> headers, Response response) {
        ...
        for (Header header : headers) {
            HeaderHandler handler = RESPONSE_HEADER_HANDLERS.get(header.getName().toLowerCase());
            if (handler != null) {
                handler.handle(header, response, headers);
            }
        }
    }

    /**
     * Extracts entity headers and updates a given representation or create an empty one when at
     * least one entity header is present.
     *
     * @param headers The headers to copy.
    ...
    public static Representation extractEntityHeaders(
            Iterable<Header> headers, Representation representation) {
        Representation result = representation == null ? new EmptyRepresentation() : representation;
        boolean entityHeaderFound = false;

        if (headers != null) {
            for (Header header : headers) {
                RepresentationHeaderHandler handler =
                        REPRESENTATION_HEADER_HANDLERS.get(header.getName().toLowerCase());
                if (handler != null) {
                    handler.handle(header, result);
    ...
    /**
     * Returns the content length of the request entity if known, {@link
    ...
    /**
     * Indicates if the given character is in the ASCII range.
     *
     * @param character The character to test.
     * @return True if the given character is in the ASCII range.
     */
    ...
    public static boolean isChunkedEncoding(Series<Header> headers) {
        ...
        if (headers != null) {
            final String header =
                    headers.getFirstValue(HeaderConstants.HEADER_TRANSFER_ENCODING, true);
    ...
    /**
     * Indicates if the given character is a comment text. It means {@link #isText(int)} returns
     * true and the character is not '(' or ')'.
     *
     * @param character The character to test.
    ...
    /**
     * Indicates if the given character is a horizontal tab.
     *
     * @param character The character to test.
     * @return True if the given character is a horizontal tab.
     */
    ...
    /**
     * Indicates if the given character is in ISO Latin 1 (8859-1) range. Note that this range is a
     * superset of ASCII and a subrange of Unicode (UTF-8).
     *
     * @param character The character to test.
    ...
    public static boolean isLinearWhiteSpace(int character) {
        return (isCarriageReturn(character)
                || isSpace(character)
    ...
    /**
     * Indicates if the given character is a quoted text. It means {@link #isText(int)} returns true
     * and {@link #isDoubleQuote(int)} returns false.
     *
     * @param character The character to test.
     * @return True if the given character is a quoted text.
     */
    ...
    /**
     * Indicates if the given character is a semicolon, the character used as a header parameter
     * separator.
     *
     * @param character The character to test.
    ...
    // Functional interface for handlers that need access to both header and response
    @FunctionalInterface
    private interface HeaderHandler {
        void handle(Header header, Response response, Series<Header> allHeaders);
    }

    // Functional interface for handlers that need access to both header and representation
    @FunctionalInterface
    private interface RepresentationHeaderHandler {
        void handle(Header header, Representation representation);
    }
