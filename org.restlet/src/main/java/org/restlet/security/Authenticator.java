...
/**
 * Filter authenticating the client sending the inbound request. Its main role is to inspect various
 * credentials provided by the client and to add related application roles to the request's {@link
 * ClientInfo} property.
 *
...
public abstract class Authenticator extends Filter {
    ...
    /**
     * Indicates if the authenticator should attempt to authenticate an already authenticated
     * client. By default, it is set to true.
     */
    private volatile boolean multiAuthenticating;

    /**
     * Indicates if the authenticator is not required to succeed. In those cases, the attached
     * Restlet is invoked. Note that authentication will be attempted independently of this property
     * unless the client is already authenticated and the {@link #isMultiAuthenticating()} prevents
     ...
     */
    protected Authenticator(Context context, boolean optional) {
     ...
     *
     * @param context The context.
     * @param multiAuthenticating Indicates if the authenticator should attempt to authenticate an
    ...
    protected int authenticated(Request request, Response response) {
        ...
        if (loggable && request.getChallengeResponse() != null) {
            getLogger()
                    .fine(
                            () ->
                                    "The authentication succeeded for the identifier \""
    ...
    /**
     * Handles the authentication by first invoking the {@link #authenticate(Request, Response)}
     * method, only if {@link #isMultiAuthenticating()} returns true and if {@link
     * ClientInfo#isAuthenticated()} returns false. If the method is invoked and returns true, the
     * {@link #authenticated(Request, Response)} is called. Otherwise, if {@link #isOptional()}
     * returns true it continues to the next Restlet or if it returns false it calls the {@link
     * #unauthenticated(Request, Response)} method.
     */
    @Override
    ...
    /**
     * Returns the enroler invoked upon successful authentication to update the subject with new
     * principals. Typically, new {@link Role} are added based on the available {@link User}
     * instances available.
     *
    ...
    /**
     * Indicates if the authenticator should attempt to authenticate an already authenticated
     * client. The client is considered authenticated if {@link ClientInfo#isAuthenticated()}
     * returns true. By default, it is set to true.
     *
     * @return True if the authenticator should attempt to authenticate an already authenticated
     *     client.
     */
    ...
    protected int unauthenticated(Request request, Response response) {
        ...
        if (request.getChallengeResponse() != null && loggable) {
            getLogger()
                    .fine(
                            () ->
                                    "The authentication failed for the identifier \""
                                            + request.getChallengeResponse().getIdentifier()
                                            + "\" using the "
                                            + request.getChallengeResponse().getScheme()
                                            + " scheme.");
