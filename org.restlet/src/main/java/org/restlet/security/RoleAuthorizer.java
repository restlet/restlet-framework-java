...
/**
 * Authorizer based on authorized and forbidden roles. Note that if no role is added to the
 * "authorizedRoles" list, then only the "forbiddenRoles" list is considered.
 *
...
public class RoleAuthorizer extends Authorizer {
    ...
    /**
     * Authorizes the request only if its subject is in one of the authorized roles and in none of
     * the forbidden ones.
     *
    ...
    public boolean authorize(Request request, Response response) {
        ...
        if (getAuthorizedRoles().isEmpty()) {
            ...
            for (Role authorizedRole : getAuthorizedRoles()) {
                authorized =
                        authorized || request.getClientInfo().getRoles().contains(authorizedRole);
            }
        }
    ...
    /**
     * Sets the modifiable list of authorized roles. This method clears the current list and adds
    ...
    /**
     * Sets the modifiable list of forbidden roles. This method clears the current list and adds all
