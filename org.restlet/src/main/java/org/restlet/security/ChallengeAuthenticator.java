...
/**
 * Authenticator based on a challenge scheme. This is typically used to support the HTTP BASIC and
 * DIGEST challenge schemes.
 *
 * @see ChallengeScheme
...
public class ChallengeAuthenticator extends Authenticator {
    ...
    /**
     * Indicates if a new challenge should be sent when invalid credentials are received (true by
     * default to conform to HTTP recommendations).
     */
     ...
     * @see #ChallengeAuthenticator(Context, boolean, ChallengeScheme, String, Verifier)
     */
    public ChallengeAuthenticator(
            Context context, boolean optional, ChallengeScheme challengeScheme, String realm) {
        this(
                context,
                optional,
                challengeScheme,
    ...
    /**
     * Authenticates the call, relying on the verifier to check the credentials provided (in general
     * an identifier and secret couple). If the credentials are valid, the next Restlet attached is
     * invoked.<br>
     * <br>
     * If the credentials are missing, then {@link #challenge(Response, boolean)} is invoked.<br>
     * <br>
     * If the credentials are invalid and if the "rechallenge" property is true then {@link
     * #challenge(Response, boolean)} is invoked. Otherwise, {@link #forbid(Response)} is invoked.
     * <br>
     * <br>
     * If the credentials are stale, then {@link #challenge(Response, boolean)} is invoked with the
     * "stale" parameter to true.<br>
     * <br>
     * At the end of the process, the {@link ClientInfo#setAuthenticated(boolean)} method is
     * invoked.
     */
    @Override
    protected boolean authenticate(Request request, Response response) {
        ...
        if (getVerifier() != null) {
            ...
            response.setStatus(
                    Status.SERVER_ERROR_INTERNAL, "Authentication failed. No verifier provided.");
        }
    ...
    /**
     * Challenges the client by adding a challenge request to the response and by setting the status
     * to {@link Status#CLIENT_ERROR_UNAUTHORIZED}.
     *
     ...
     * authentication is required, the challenge method is invoked, otherwise the call status is set
    ...
    public void forbid(Response response) {
        ...
        if (loggable) {
            getLogger()
                    .log(
                            Level.FINE,
                            "Authentication or authorization failed for this URI: {0}",
    ...
    /**
     * Indicates if a new challenge should be sent when invalid credentials are received (true by
     * default to conform to HTTP recommendations). If set to false, upon reception of invalid
     * credentials, the method {@link #forbid(Response)} will be called.
     *
