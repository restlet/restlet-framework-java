 ...
 * This class changes the semantics of the {@link Resource#getRequest()} and {@link
 * Resource#getResponse()} methods. Since a clientResource may receive severals responses for a
 * single request (in case of interim response), the {@link #getResponse()} method returns the last
 * received response object. The Request object returned by the {@link #getRequest()} is actually a
 * prototype which is cloned (except the representation) just before the {@link #handle()} method is
 * called.<br>
 * Users must be aware that by most representations can only be read or written once. Some others,
 * such as {@link StringRepresentation} stored the entity in memory which can be read several times
 * but has the drawback to consume memory.<br>
 * Concurrency note: instances of the class are not designed to be shared among several threads. If
 * thread-safety is necessary, consider using the lower-level {@link Client} class instead.
 *
...
public class ClientResource extends Resource {
    ...
    /**
     * Creates a client resource that proxy calls to the given Java interface into Restlet method
     * calls. It basically creates a new instance of {@link ClientResource} and invokes the {@link
     * #wrap(Class)} method.
     *
    ...
    public static <T> T create(
            Context context, Reference reference, Class<? extends T> resourceInterface) {
    ...
    /**
     * Indicates the maximum number of redirections that can be automatically followed for a single
     * call.
     */
    ...
    /**
     * Indicates if transient or unknown size response entities should be buffered after being
     * received.
     */
    ...
    public ClientResource(Context context, Request request, Response response) {
        ...
        // See the constructor ClientResource(Context, Method, Reference)
    ...
    /**
     * Updates the client preferences to accept the given metadata (media types, character sets,
     * etc.) with a 1.0 quality in addition to existing ones.
     *
    ...
    /**
     * Updates the client preferences to accept the given metadata (media types, character sets,
     * etc.) with a given quality in addition to existing ones.
     *
     * @param metadata The metadata to accept.
    ...
    /**
     * Adds a parameter to the query component. The name and value are automatically encoded if
     * necessary.
     *
    ...
    /**
     * Adds several parameters to the query component. The name and value are automatically encoded
     * if necessary.
     *
    ...
    /**
     * Adds a segment at the end of the path. If the current path doesn't end with a slash
     * character, one is inserted before the new segment value. The value is automatically encoded
     * if necessary.
     *
    ...
    protected Uniform createNext() {
        ...
        if (result == null) {
            ...
            if (protocol != null) {
                org.restlet.engine.util.TemplateDispatcher dispatcher =
                        new org.restlet.engine.util.TemplateDispatcher();
    ...
    /**
     * By default, it throws a new resource exception. Call {@link #doError(org.restlet.data.Status,
    ...
    /**
     * By default, it throws a new resource exception. This can be overridden to provide a different
     * behavior.
     *
     * @param errorStatus The error status received.
     * @param request The associated request.
    ...
    /**
     * Releases the resource by stopping any connector automatically created and associated with the
     * "next" property (see {@link #getNext()} method.
     */
    @Override
    protected void doRelease() throws ResourceException {
        if ((getNext() != null) && this.nextCreated) {
            if (getNext() instanceof Restlet restlet) {
                try {
                    restlet.stop();
                } catch (Exception e) {
    ...
    /**
     * Represents the resource using content negotiation to select the best variant based on the
     * client preferences. Note that the client preferences will be automatically adjusted, but only
     * for this request. If you want to change them once for all, you can use the {@link
     * #getClientInfo()} method.<br>
     * <br>
    ...
    /**
     * Represents the resource in the given object class. Note that the client preferences will be
     * automatically adjusted, but only for this request. If you want to change them once for all,
     * you can use the {@link #getClientInfo()} method.<br>
     * <br>
     * If a success status is not returned, then a resource exception is thrown.
     *
     * @param <T> The expected type for the response entity.
    ...
    /**
     * Represents the resource using a given media type. Note that the client preferences will be
     * automatically adjusted, but only for this request. If you want to change them once for all,
     * you can use the {@link #getClientInfo()} method.<br>
     * <br>
     * If a success status is not returned, then a resource exception is thrown.
    ...
    /**
     * Returns the attribute value by looking up the given name in the response attributes maps. The
     * toString() method is then invoked on the attribute value.
     *
     ...
     * resource is defined in the sense of hierarchical URIs. If the resource URI is not
     * hierarchical, then an exception is thrown.
     *
    ...
    public ClientResource getChild(Reference relativeRef) throws ResourceException {
        ...
        if ((relativeRef != null) && relativeRef.isRelative()) {
            ...
            result.setReference(
                    new Reference(getReference().getTargetRef(), relativeRef).getTargetRef());
        } else {
    ...
    /**
     * Wraps the child client resource to proxy call to the given Java interface into Restlet method
     * calls. The child resource is defined in the sense of hierarchical URIs. If the resource URI
     * is not hierarchical, then an exception is thrown.
     *
     * @param <T> The proxied interface.
     * @param relativeRef The URI reference of the child resource relatively to the current resource
     *     seen as the parent resource.
     ...
     * hierarchical, then an exception is thrown.
     *
     * @param relativeUri The URI of the child resource relatively to the current resource seen as
     *     the parent resource.
     * @return The child resource.
     * @throws ResourceException
     */
    ...
    /**
     * Returns the maximum number of redirections that can be automatically followed for a single
     * call. The default value is 10.
     *
     * @return The maximum number of redirections that can be automatically followed for a single
     *     call.
     */
    ...
    /**
     * Returns the callback invoked on response reception. If the value is not null, then the
     * associated request will be executed asynchronously.
     *
    ...
    /**
     * Returns the parent resource. The parent resource is defined in the sense of hierarchical
     * URIs. If the resource URI is not hierarchical, then an exception is thrown.
     *
    ...
    /**
     * Wraps the parent client resource to proxy call to the given Java interface into Restlet
     * method calls. The parent resource is defined in the sense of hierarchical URIs. If the
     * resource URI is not hierarchical, then an exception is thrown.
     *
     * @param <T> The proxied interface.
     * @param resourceInterface The annotated resource interface class to proxy.
     * @return The proxy instance.
    ...
    /**
     * Returns the number of retry attempts before reporting an error. The default value is 2.
     *
    ...
    /**
     * Returns the delay in milliseconds between two retry attempts. The default value is 2 seconds.
     *
    ...
    /**
     * Handles the call by invoking the next handler. The prototype request is retrieved via {@link
     * #getRequest()} and cloned and the response is set as the latest with {@link
     * #setResponse(Response)}. If necessary the {@link #setNext(Uniform)} method is called as well
     * with a {@link Client} instance matching the request protocol.
     *
     * @return The optional response entity.
     * @see #getNext()
     */
    ...
    /**
     * Handles an object entity. Automatically serializes the object using the {@link
    ...
    protected <T> T handle(Method method, Object entity, Class<T> resultClass)
        ...
        if (entity != null) {
            try {
                List<Variant> entityVariants = cs.getVariants(entity.getClass(), null);
                request.setEntity(
                        toRepresentation(
                                entity,
                                getConnegService()
                                        .getPreferredVariant(
                                                entityVariants, request, getMetadataService())));
            } catch (IOException e) {
                throw new ResourceException(e);
            }
        } else {
    ...
    protected void handle(
        ...
        if (isRetryOnError()
            ...
            redirectRequestIfNecessary(request, response, references, retryAttempt, next);
        }
    ...
    /**
     * Checks if the request should be redirected, then prepares the new request, and handles the
     * redirection.
     */
    private void redirectRequestIfNecessary(
    ...
    public Response handleOutbound(Request request) {
        ...
        final Uniform nextUniform = getNext();

        if (nextUniform != null) {
            // Effectively handle the call
            handle(request, response, null, 0, nextUniform);
    ...
    /**
     * Represents the resource using content negotiation to select the best variant based on the
     * client preferences. This method is identical to {@link #get()} but doesn't return the actual
     * content of the representation, only its metadata.<br>
     * <br>
     * Note that the client preferences will be automatically adjusted, but only for this request.
     * If you want to change them once for all, you can use the {@link #getClientInfo()} method.<br>
     * <br>
     * If a success status is not returned, then a resource exception is thrown.
     *
     * @return The best representation.
     * @throws ResourceException
     * @see <a href= "http://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.4">HTTP HEAD
     *     method</a>
     */
    ...
    /**
     * Represents the resource using a given media type. This method is identical to {@link
     * #get(MediaType)} but doesn't return the actual content of the representation, only its
     * metadata.<br>
     * <br>
     * Note that the client preferences will be automatically adjusted, but only for this request.
     * If you want to change them once for all, you can use the {@link #getClientInfo()} method.<br>
    ...
    /**
     * Indicates if transient or unknown size response entities should be buffered after being
     * received. This is useful to increase the chance of being able to resubmit a failed request
     * due to network error, or to prevent chunked encoding from being used an HTTP connector.
     *
     * @return True if transient response entities should be buffered after being received.
     */
    ...
    /**
     * Indicates if transient or unknown size response entities should be buffered after being
     * received. This is useful to be able to systematically reuse and process a response entity
     * several times after retrieval.
     *
     * @return True if transient response entities should be buffered after being received.
     */
    ...
    /**
     * Describes the resource using content negotiation to select the best variant based on the
     * client preferences. If a success status is not returned, then a resource exception is thrown.
     *
     * @return The best description.
     * @throws ResourceException
     * @see <a href= "http://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.2">HTTP OPTIONS
     *     method</a>
     */
    ...
    /**
     * Describes the resource using a given media type. If a success status is not returned, then a
    ...
    /**
     * Patches a resource with the given object as a delta state. Automatically serializes the
     * object using the {@link org.restlet.service.ConverterService}.
     *
    ...
    /**
     * Patches a resource with the given object as delta state. Automatically serializes the object
     * using the {@link org.restlet.service.ConverterService}.
     *
     * @param entity The object entity containing the patch.
     * @param mediaType The media type of the representation to retrieve.
     * @return The response object entity.
    ...
    /**
     * Patches a resource with the given representation as delta state. If a success status is not
    ...
    /**
     * Posts an object entity. Automatically serializes the object using the {@link
     * org.restlet.service.ConverterService}.
     *
     * @param entity The object entity to post.
     * @return The optional result entity.
    ...
    /**
     * Posts a representation. If a success status is not returned, then a resource exception is
     * thrown.
     *
    ...
    /**
     * Puts an object entity. Automatically serializes the object using the {@link
     * org.restlet.service.ConverterService}.
     *
    ...
    /**
     * Creates or updates a resource with the given representation as new state to be stored. If a
    ...
    /**
     * Effectively redirects a client call. By default, it checks for infinite loops and unavailable
     * entities, the references list is updated and the {@link #handle(Request, Response, List, int,
     * Uniform)} method invoked.
     *
     * @param request The request to send.
     * @param response The response to modify.
     * @param references The references that caused a redirection to prevent infinite loops.
    ...
    protected void redirect(
        ...
        if ((references != null) && references.contains(newTargetRef)) {
            getLogger()
                    .warning(() -> "Infinite redirection loop detected with URI: " + newTargetRef);
    ...
    /**
     * Effectively retries a failed client call. By default, it sleeps before the retry attempt and
     ...
     *
     * @param challengeResponse The authentication response sent by a client to an origin server.
    ...
    public void setChallengeResponse(
            ChallengeScheme scheme, final String identifier, String secret) {
    ...
    /**
     * Sets the host reference using a URI string.
     *
     ...
     *
     * @param maxRedirects The maximum number of redirections that can be automatically followed for
     *     a single call.
     */
     ...
     *
     * <p>In addition, this method will set the context of the next Restlet if it is null by passing
     * a reference to its own context.
     *
     * @param next The next handler.
     */
    public void setNext(org.restlet.Uniform next) {
        if (next instanceof final Restlet nextRestlet && nextRestlet.getContext() == null) {
    ...
    /**
     * Sets the callback invoked on response reception. If the value is not null, then the
     * associated request will be executed asynchronously.
     *
     ...
     *
     * @param challengeResponse The proxy authentication response sent by a client to an origin
     *     server.
    ...
    /**
     * Sets the proxy authentication response sent by a client to an origin server given a scheme,
     * identifier, and secret.
     *
     * @param scheme The challenge scheme.
     * @param identifier The user identifier, such as a login name or an access key.
     * @param secret The user secret, such as a password or a secret key.
     */
    public void setProxyChallengeResponse(
            ChallengeScheme scheme, final String identifier, String secret) {
    ...
    /**
     * Sets the resource's reference. If the reference is relative, it will be resolved as an
    ...
    /**
     * Sets the resource's reference using a URI string. Note that the URI can be either absolute or
     * relative to the context's base reference.
     *
    ...
    /**
     * Sets the referrer reference if available using a URI string.
     *
    ...
    /**
     * Wraps the client resource to proxy calls to the given Java interface into Restlet method
     * calls. Use the {@link org.restlet.engine.Engine} classloader to generate the proxy.
     *
     * @param <T>
     * @param resourceInterface The annotated resource interface class to proxy.
     * @return The proxy instance.
     ...
     * @param <T>
     * @param resourceInterface The annotated resource interface class to proxy.
     * @param classLoader The classloader used to instantiate the dynamic proxy.
     * @return The proxy instance.
     */
    ...
    public <T> T wrap(Class<? extends T> resourceInterface, ClassLoader classLoader) {
        ...
        java.lang.reflect.InvocationHandler h =
                new org.restlet.engine.resource.ClientInvocationHandler<T>(this, resourceInterface);
