...
/**
 * Base resource class exposing the uniform REST interface. Intended conceptual target of a
 * hypertext reference. A uniform resource encapsulates a {@link Context}, a {@link Request} and a
 * {@link Response}, corresponding to a specific target resource.<br>
 * <br>
 * It also defines a precise life cycle. First, the instance is created and the {@link
 * #init(Context, Request, Response)} method is invoked. If you need to do some additional
 * initialization, you should override the {@link #doInit()} method.<br>
 * <br>
 * Then, the abstract {@link #handle()} method can be invoked. For concrete behavior, see the {@link
 * ClientResource} and {@link ServerResource} subclasses. Note that the state of the resource can be
 * changed several times and the {@link #handle()} method called more than once, but always by the
 * same thread.<br>
 * <br>
 * Finally, the final {@link #release()} method can be called to clean up the resource, with a
 * chance for the developer to do some additional cleanup by overriding the {@link #doRelease()}
 * method.<br>
 * <br>
 * Note also that throwable raised such as {@link Error} and {@link Exception} can be caught in a
 * single point by overriding the {@link #doCatch(Throwable)} method.<br>
 * <br>
 * "The central feature that distinguishes the REST architectural style from other network-based
 * styles is its emphasis on a uniform interface between components. By applying the software
 * engineering principle of generality to the component interface, the overall system architecture
 * is simplified and the visibility of interactions is improved. Implementations are decoupled from
 * the services they provide, which encourages independent evolvability." Roy T. Fielding<br>
 * <br>
 ...
 * Resource} is created for each call handled and accessed by only one thread at a time.
 *
 * @see <a href= "http://roy.gbiv.com/pubs/dissertation/rest_arch_style.htm#sec_5_1_5">Source
 *     dissertation</a>
...
public abstract class Resource {
    ...
    /**
     * Invoked when a {@link Throwable} is caught during initialization, handling, or releasing.
     *
    ...
    /**
     * Setup method that can be overridden to initialize the state of the resource. By default, it
     * does nothing.
     *
    ...
    /**
     * Cleanup method that can be overridden to release the state of the resource. By default, it
     * does nothing.
     *
     * @see #release()
     */
    ...
    /**
     * Returns the set of methods allowed for the current client by the resource. The result can
     * vary based on the client's user agent, authentication and authorization data provided by the
     * client.
     *
    ...
    /**
     * Returns the parent application. If it wasn't set, it attempts to retrieve the current one via
     * {@link org.restlet.Application#getCurrent()} if it exists, or instantiates a new one as a
     * last resort.
     *
    ...
    /**
     * Returns the attribute value by looking up the given name in the request or response
     * attributes maps. This is typically used for variables that are declared in the URI template
     * used to route the call to this resource.
     *
     ...
     * can be left empty. Creates a new instance if no one has been set.
     *
     ...
     * for URNs and other URIs that don't contain host information.
     *
    ...
    public int getMaxForwards() {
        return getRequest() == null ? 0 : getRequest().getMaxForwards();
    }
    ...
    /**
     * Returns the protocol by first returning the resourceRef.schemeProtocol property if it is set,
     * or the baseRef.schemeProtocol property otherwise.
     *
    ...
    /**
     * Returns the list of proxy authentication requests sent by an origin server to a client. If
     * none is available, an empty list is returned.
     *
    ...
    /**
     * Returns the resource reference's optional query. Note that modifications to the returned
     * {@link Form} object aren't reported to the underlying reference.
     *
    ...
    /**
     * Returns the request cache directives. Note that when used with HTTP connectors, this property
     * maps to the "Cache-Control" header.
     *
    ...
    /**
     * Returns the response cache directives. Note that when used with HTTP connectors, this
     * property maps to the "Cache-Control" header.
     *
     * @return The cache directives.
     */
    ...
    /**
     * Initialization method setting the environment of the current resource instance. It the calls
     * the {@link #doInit()} method that can be overridden.
     *
    ...
    /**
     * Indicates if the message was or will be exchanged confidentially, for example, via an
    ...
    /**
     * Sets the query value for the named parameter. If no query is defined, it creates one. If the
     * same parameter exists, it replaces it altogether.
     *
     * @param name The query parameter name.
    ...
    /**
     * Converts a representation into a Java object. Leverages the {@link
