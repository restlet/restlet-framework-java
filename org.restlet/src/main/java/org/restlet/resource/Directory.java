...
/**
 * Finder mapping a directory of local resources. Those resources have representations accessed by
 * the file system, the class loaders, or other URI-accessible protocols. Here is some sample code
 * illustrating how to attach a directory to a router:<br>
 *
 * <pre>
 ...
 * </pre>
 *
 * An automatic content negotiation mechanism (similar to the one in Apache HTTP Server) is used to
 * select the best representation of a resource based on the available variants and on the client
 * capabilities and preferences.<br>
 * <br>
 * The directory can be used in read-only or modifiable mode. In the latter case, you need to set
 * the "modifiable" property to true. The currently supported methods are PUT and DELETE.<br>
 * <br>
 * When no index is available in a given directory, a representation can be automatically generated
 * by the {@link #getIndexRepresentation(Variant, ReferenceList)} method, unless the
 * "listingAllowed" property is turned off. You can even customize the way the index entries are
 * sorted by using the {@link #setComparator(Comparator)} method. The default sorting uses the
 * friendly Alphanum algorithm based on David Koelle's <a
 * href="http://www.davekoelle.com/alphanum.html">original idea</a>, using a different and faster
 * implementation contributed by Rob Heittman.<br>
 * <br>
...
public class Directory extends Finder {
     ...
     *     <br>
     *     If you serve files from the file system, use file:// URIs and make sure that you register
     *     a FILE connector with your parent Component. On Windows, make sure that you add enough
     *     slash characters at the beginning, for example: file:///c:/dir/file<br>
     *     <br>
     *     If you serve files from a class loader, use clap:// URIs and make sure that you register
     *     a CLAP connector with your parent Component.<br>
     *     <br>
     */
    ...
    /**
     * Returns the reference comparator used to sort index pages. The default implementation used a
     * friendly alphanum sorting.
     *
    ...
    /**
     * Returns the variant representations of a directory index. This method can be subclassed to
     * provide alternative representations.
     *
     * <p>By default, it returns a simple HTML document and a textual URI list as variants. Note
     * that a new instance of the list is created for each call.
     *
    ...
    /**
     * Indicates if modifications to local resources (most likely files) are allowed. Returns false
     * by default.
     *
    ...
    /**
     * Indicates if the best content is automatically negotiated. The default value is true.
     *
     ...
     * Indicates if the display of directory listings is allowed when no index file is found.
     *
     * @param listingAllowed True if the display of directory listings is allowed when no index file
     *     is found.
     */
    ...
    /**
     * Sets the root URI from which the relative resource URIs will be looked up.
     *
    ...
    /**
     * Sets the reference comparator based on the more friendly "Alphanum Algorithm" created by
     * David Koelle. The internal implementation used is based on an optimized public domain
     * implementation provided by Rob Heittman from the Solertium Corporation.
     *
     * @see <a href="http://www.davekoelle.com/alphanum.html">The original Alphanum Algorithm from
     *     David Koelle</a>
     * @see #setComparator(Comparator)
     */
