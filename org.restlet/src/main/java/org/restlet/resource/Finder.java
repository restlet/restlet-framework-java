...
/**
 * Restlet that can find the target server resource that will effectively handle incoming calls. By
 * default, based on a given {@link ServerResource} subclass available via the {@link
 * #getTargetClass()} method, it automatically instantiates for each incoming call the target
 * resource class using its default constructor and invoking the {@link ServerResource#init(Context,
 * Request, Response)} method.<br>
 * <br>
 * Once the target has been created, the call is automatically dispatched to the {@link
 * ServerResource#handle()} method.<br>
 * <br>
 * Once the call is handled, the {@link ServerResource#release()} method is invoked to permit
 * cleanup actions.<br>
 * <br>
...
public class Finder extends Restlet {
    ...
    public static Finder createFinder(
        ...
        if (finderClass != null) {
            try {
                Constructor<? extends Finder> constructor =
                        finderClass.getConstructor(Context.class, Class.class);
    ...
    /**
     * Creates a new instance of a given {@link ServerResource} subclass. Note that {@link Error}
     * and {@link RuntimeException} thrown by {@link ServerResource} constructors are re-thrown by
     * this method. Other exception is caught and logged.
     *
     * @param targetClass The target {@link ServerResource} subclass.
    ...
    public ServerResource create(
            Class<? extends ServerResource> targetClass, Request request, Response response) {
    ...
    /**
     * Creates a new instance of the {@link ServerResource} subclass designated by the "targetClass"
     * property. The default behavior is to invoke the {@link #create(Class, Request, Response)}
     * with the "targetClass" property as a parameter.
     *
     * @param request The request to handle.
     * @param response The response to modify.
     * @return The created resource or null.
    ...
    /**
     * Finds the target {@link ServerResource} if available. The default behavior is to invoke the
     * {@link #create(Request, Response)} method.
     *
     * @param request The request to handle.
     * @param response The response to modify.
    ...
    /**
     * Returns the target resource class which must be either a subclass of {@link ServerResource}.
     *
    ...
    /**
     * Sets the target resource class which must be a subclass of {@link ServerResource}.
     *
     * @param targetClass The target resource class. It must be a subclass of {@link
     *     ServerResource}.
     */
