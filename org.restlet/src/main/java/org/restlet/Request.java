 ...
 * by {@link Restlet}s. This request can also be processed by a chain of Restlets, on both client
 * and server sides. Requests are uniform across all types of connectors, protocols, and components.
 *
...
public class Request extends Message {
    ...
    /**
     * Returns the request associated with the current thread. This is reusing the {@link
     * Response#getCurrent()} method.<br>
     * <br>
     * Warning: this method should only be used under duress. You should by default prefer getting
     * the current context using methods such as {@link org.restlet.resource.Resource#getRequest()}.
     *
    ...
    /**
     * Used when issuing a preflight CORS request to let the origin server knows what headers the
     * client is willing to send in future request to this resource.
     */
    private volatile Set<String> accessControlRequestHeaders;

    /**
     * Used when issuing a preflight CORS request to let the origin server knows what method the
     * client is willing to send in a future request to this resource.
     */
    ...
    public Request(Request request) {
        ...
        copyClientInfo(request);

        copyConditions(request);
        ...
        this.originalRef =
                (request.getOriginalRef() == null) ? null : new Reference(request.getOriginalRef());
        ...
        this.referrerRef =
                (request.getReferrerRef() == null) ? null : new Reference(request.getReferrerRef());
    ...
    private void copyConditions(final Request request) {
    ...
    private void copyClientInfo(final Request request) {
    ...
    /**
     * Ask the connector to attempt to abort the related network connection, for example immediately
     * closing the socket.
     *
    ...
    /**
     * Asks the server connector to immediately commit the given response associated with this
     * request, making it ready to be sent back to the client. Note that all server connectors don't
     * necessarily support this feature.
     *
    ...
    /**
     * Returns the modifiable set of headers the client is willing to send in future request to this
     * resource. Used when issuing a preflight CORS request to let the origin server know what
     * headers will be sent later.<br>
     * Note that when used with HTTP connectors, this property maps to the
     * "Access-Control-Allow-Headers" header.
     *
     * @return The headers the client is willing to send in future request to this resource. Useful
     *     for CORS support.
     */
    public Set<String> getAccessControlRequestHeaders() {
        ...
        if (a == null) {
            synchronized (this) {
                ...
                if (a == null) {
                    this.accessControlRequestHeaders = a = new CopyOnWriteArraySet<>();
                }
            }
        }
        return a;
    ...
    /**
     * Returns the method the client is willing to use in future request to this resource. Used when
     * issuing a preflight CORS request to let the origin server know what method will be sent
     * later.<br>
     * Note that when used with HTTP connectors, this property maps to the
     * "Access-Control-Request-Method" header.
     *
     * @return The method the client is willing to send in a future request to this resource. Useful
     *     for CORS support.
     */
    ...
    /**
     * Returns the authentication response sent by a client to an origin server. Note that when used
     * with HTTP connectors, this property maps to the "Authorization" header.
     *
    ...
    /**
     * Returns the modifiable conditions applying to this request. Creates a new instance if no one
     * has been set.
     *
    ...
    /**
     * Returns the modifiable series of cookies provided by the client. Creates a new instance if no
     * one has been set.<br>
     * <br>
    ...
    public Series<Cookie> getCookies() {
        ...
        if (c == null) {
            synchronized (this) {
                c = this.cookies;
                if (c == null) {
                    this.cookies = c = new Series<>(Cookie.class);
                }
            }
        }
        return c;
    ...
    /**
     * Returns the host reference. This may be different from the resourceRef's host, for example,
     * for URNs and other URIs that don't contain host information.<br>
     * <br>
    ...
    /**
     * Returns the original reference as requested by the client. Note that this property is not
     * used during request routing. See the {@link #getResourceRef()} method for details.
     *
    ...
    public Protocol getProtocol() {
        ...
        if ((result == null) && (getResourceRef() != null)) {
            ...
            // Fallback: look at a base reference scheme
            if (result == null) {
                result =
                        (getResourceRef().getBaseRef() != null)
    ...
    /**
     * Returns the authentication response sent by a client to a proxy. Note that when used with
     * HTTP connectors, this property maps to the "Proxy-Authorization" header.
     *
    ...
    /**
     * Returns the ranges to return from the target resource's representation. Note that when used
     * with HTTP connectors, this property maps to the "Range" header.
     *
    ...
    /**
     * Returns the referrer reference if available. Note that when used with HTTP connectors, this
     * property maps to the "Referer" header.
     *
    ...
    /**
     * Returns the reference of the target resource. This reference is especially important during
     * routing, dispatching, and resource finding. During such processing, its base reference is
     * constantly updated to reflect the reference of the parent Restlet or resource and the
     * remaining part of the URI that must be routed or analyzed.
     *
     * <p>If you need to get the URI reference originally requested by the client, then you should
     * use the {@link #getOriginalRef()} method instead. Also, note that besides the update of its
     * base property, the resource reference can be modified during the request processing.
     *
     * <p>For example, the {@link org.restlet.service.TunnelService} associated with an application
     * can extract some special extensions or query parameters and replace them by semantically
     * equivalent properties on the request object. Therefore, the resource reference can become
     * different from the original reference.
     *
     * <p>Finally, when sending out requests via a dispatcher such as {@link
     * Context#getClientDispatcher()} or {@link Context#getServerDispatcher()}, if the reference
     * contains URI template variables, those variables are automatically resolved using the
    ...
    /**
     * Indicates if the request is asynchronous. The test consist in verifying that the {@link
     * #getOnResponse()} method returns a callback object.
     *
    ...
    /**
     * Implemented based on the {@link Protocol#isConfidential()} method for the request's protocol
    ...
    /**
     * Indicates if a content is available and can be sent. Several conditions must be met: the
     * method must allow the sending of content, the content must exist and have some available
     * data.
     *
    ...
    public boolean isExpectingResponse() {
        return getMethod() != null && getMethod().isReplying();
    }
    ...
    /**
     * Indicates if the request is synchronous. The test consist in verifying that the {@link
     * #getOnResponse()} method returns null.
     *
     * @return True if the request is synchronous.
     */
    ...
    /**
     * Sets the set of headers the client is willing to use in future request to this resource. Used
     * when issuing a preflight CORS request to let the origin server know what headers will be sent
     * later.<br>
     * Note that when used with HTTP connectors, this property maps to the
     * "Access-Control-Request-Method" header.
     *
     * @param accessControlRequestHeaders The set of headers the client is willing to send in a
     *     future request to this resource. Useful for CORS support.
     */
    ...
    /**
     * Sets the method the client is willing to use in future request to this resource. Used when
     * issuing a preflight CORS request to let the origin server know what method will be sent
     * later.<br>
     * Note that when used with HTTP connectors, this property maps to the
     * "Access-Control-Request-Method" header.
     *
     * @param accessControlRequestMethod The method the client is willing to send in future request
     *     to this resource. Useful for CORS support.
     */
    ...
    /**
     * Sets the authentication response sent by a client to an origin server. Note that when used
     * with HTTP connectors, this property maps to the "Authorization" header.
     *
    ...
    /**
     * Sets the modifiable series of cookies provided by the client. Note that when used with HTTP
     * connectors, this property maps to the "Cookie" header. This method clears the current series
     * and adds all entries in the parameter series.
     *
    ...
    /**
     * Sets the host reference. Note that when used with HTTP connectors, this property maps to the
     * "Host" header.
     *
    ...
    /**
     * Sets the host reference using a URI string. Note that when used with HTTP connectors, this
    ...
    /**
     * Sets the authentication response sent by a client to a proxy. Note that when used with HTTP
     * connectors, this property maps to the "Proxy-Authorization" header.
     *
    ...
    /**
     * Sets the modifiable list of ranges to return from the target resource's representation. Note
     * that when used with HTTP connectors, this property maps to the "Range" header. This method
    ...
    /**
     * Sets the referrer reference if available. Note that when used with HTTP connectors, this
     * property maps to the "Referer" header.
     *
    ...
    /**
     * Sets the referrer reference if available using a URI string. Note that when used with HTTP
    ...
    /**
     * Sets the target resource reference using a URI string. Note that the URI can be either
    ...
    public String toString() {
        final String methodAsString = (getMethod() == null) ? "" : getMethod().toString();
        final String resourceRefAsString =
                (getResourceRef() == null) ? "" : getResourceRef().toString();
        final String protocolAsString;
        ...
        if (getProtocol() == null) {
            ...
            protocolAsString = getProtocol().getName() + protocolVersion;
        }

        return methodAsString + " " + resourceRefAsString + " " + protocolAsString;
    }
}
