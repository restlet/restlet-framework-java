...
/**
 * Application service converting between representation and regular Java objects. The conversion
 * can work in both directions. Actual converters can be plugged into the engine to support this
 * service.<br>
 * <br>
 * Root object classes used for conversion shouldn't be generic classes, otherwise important
 * contextual type information will be missing at runtime due to Java type erasure mechanism. If
 * needed, create a fully resolved subclasses and/or a container classes.
 *
...
public class ConverterService extends Service {
    ...
    /**
     * Applies a patch representation to an initial representation to obtain a modified one. The
     * patch must have a recognized media type in order for the {@link ConverterService} to be able
     * to process it.
     *
     ...
     * @throws IOException
     */
    public Representation applyPatch(Representation initial, Representation patch)
            throws IOException {
    ...
    /**
     * Creates a patch representation by calculating a diff between initial and modified
     * representations.
     *
     * @param initial The initial representation.
     * @param modified The modified representation.
     * @return The patch representation able to convert the initial representation into the modified
     *     representation.
     * @throws IOException
     */
    public Representation createPatch(Representation initial, Representation modified)
            throws IOException {
     ...
     *
     * @param representationType The representation media type or null for all supported patch
     *     types.
     ...
     */
    public List<Variant> getVariants(Class<?> source, Variant target) {
        final List<VariantInfo> variants = ConverterUtils.getVariants(source, target);
        return (variants == null) ? null : new ArrayList<>(variants);
    }

    /**
     * Reverts a patch representation from a modified representation to get the initial one. The
     * patch must have a recognized media type in order for the {@link ConverterService} to be able
     * to process it.
     *
     * @param modified The modified representation from which the patch must be reverted.
     * @param patch The patch representation to revert.
     * @return The initial representation.
     * @throws IOException
     */
    public Representation revertPatch(Representation modified, Representation patch)
            throws IOException {
        ...
        if (ch == null) {
            if (loggable) {
                Context.getCurrentLogger()
                        .log(
                                Level.WARNING,
                                "Unable to find a converter for this representation : {0}",
                                source);
            }
        } else {
    ...
    /**
     * Converts a regular Java object into a Representation. The converter will use the preferred
     * variant of the selected converter.
     *
        ...
        if (ch == null) {
            if (loggable) {
                Context.getCurrentLogger()
                        .log(
                                Level.WARNING,
                                "Unable to find a converter for this object : {0}",
                                source);
            }
        } else {
            if (loggable && Context.getCurrentLogger().isLoggable(Level.FINE)) {
            ...
            target = ensureNonNullTargetVariant(target, ch, source, resource);

            result = ch.toRepresentation(source, target, resource);

            if (result != null) {
                // Copy the variant metadata if necessary
                copyTargetVariantMetadataIfNecessary(result, target, resource);
            }
        }
    ...
    private static void copyTargetVariantMetadataIfNecessary(
            final Representation result, final Variant target, final Resource resource) {
    ...
    /**
     * Ensures that the target variant is not null. If it is null, tries to find a preferred variant
     * based on the available variants for the source object class and the request metadata.
     *
     * @return either the target variant or a new variant instance.
     */
    private static Variant ensureNonNullTargetVariant(
    ...
    /**
     * Updates the media type preferences with available conversion capabilities for the given
     * entity class.
     *
