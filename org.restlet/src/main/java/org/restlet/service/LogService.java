...
/**
 * Service providing access logging service. The implementation is fully based on the standard
 * logging mechanism introduced in JDK 1.4.<br>
 * <br>
 * The default access log format follows the <a href="http://www.w3.org/TR/WD-logfile.html">W3C
 * Extended Log File Format</a> with the following fields used: <br>
 *
 * <ol>
 *   <li>Date (YYYY-MM-DD)
 *   <li>Time (HH:MM:SS)
 *   <li>Client address (IP)
 *   <li>Remote user identifier (see RFC 1413)
 *   <li>Server address (IP)
 *   <li>Server port
 *   <li>Method (GET|POST|...)
 *   <li>Resource reference path (including the leading slash)
 *   <li>Resource reference query (excluding the leading question mark)
 *   <li>Response status code
 *   <li>Number of bytes sent
 *   <li>Number of bytes received
 *   <li>Time to serve the request (in milliseconds)
 *   <li>Host reference
 *   <li>Client agent name
 *   <li>Referrer reference
 * </ol>
 *
 * <br>
 * If you use <a href="https://github.com/fabianbuechler/analog">Analog</a> to generate your log
 * reports, and if you use the default log format, then you can simply specify this string as a
 * value of the LOGFORMAT command:
 ...
 * {@link org.restlet.routing.Template}. <br>
 *
 * @see <a href=
...
public class LogService extends Service {
    ...
    protected String getDefaultResponseLogMessage(Response response, int duration) {
        ...
        if (isIdentityCheck()) {
            org.restlet.engine.log.IdentClient ic =
                    new org.restlet.engine.log.IdentClient(
        ...
        String resourceQuery =
                (request.getResourceRef() == null) ? "-" : request.getResourceRef().getQuery();
        ...
        try {
            ...
            Engine.getLogger(LogService.class)
                    .log(Level.SEVERE, "Cannot retrieve size of request's entity", t);
            sb.append("-");
        }
        ...
        sb.append('\t');
        sb.append(
                (request.getReferrerRef() == null)
                        ? "-"
                        : request.getReferrerRef().getIdentifier());

        return sb.toString();
    }

    /**
     * Returns the URI template of loggable resource references. Returns null by default, meaning
     * all requests are loggable, independent of their target resource URI reference.
     *
    ...
    /**
     * Returns the name of the JDK's logger to use when logging access calls. The default name will
     * follow this pattern: "org.restlet.MyComponent.LogService", where "MyComponent" will
     * correspond to the simple class name of your component subclass or to the base "Component"
     * class.
     *
    ...
    /**
     * Format an access log entry. If the log template property isn't provided, then a default IIS
     * like format is used.
     *
     * @param response The response to log.
    ...
    /**
     * Indicates if the identity check (as specified by RFC1413) is enabled. The default value is
     * false.
     *
    ...
    /**
     * Indicates if the call should be logged during the processing chain. By default, it tries to
     * match the request URI with the {@link #getLoggableTemplate()} URI template otherwise is
     * returns true.
     *
    ...
    public boolean isLoggable(Request request) {
        return getLoggableTemplate() == null
                || getLoggableTemplate().match(request.getResourceRef().getTargetRef().toString())
                        > 0;
    }
    ...
    /**
     * Starts the log service by attempting to read the log properties if the {@link
    ...
    public synchronized void start() throws Exception {
        ...
        this.responseLogTemplate =
                (getResponseLogFormat() == null) ? null : new Template(getResponseLogFormat());

        if (getLogPropertiesRef() != null) {
            Representation logProperties =
                    new ClientResource(getContext(), getLogPropertiesRef()).get();

            if (logProperties != null) {
                java.util.logging.LogManager.getLogManager()
                        .readConfiguration(logProperties.getStream());
            }
        }
    }
}
