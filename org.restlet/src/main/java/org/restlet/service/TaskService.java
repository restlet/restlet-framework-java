...
/**
 * Application service capable of running and scheduling tasks asynchronously. The service instance
 * returned will not invoke the runnable task in the current thread.<br>
 * <br>
 * In addition to allowing pooling, this method will ensure that the threads executing the tasks
 * will have the thread local variables copied from the calling thread. This will ensure that call
 * to static methods like {@link Application#getCurrent()} still work.<br>
 * <br>
 * Also, note that this executor service will be shared among all Restlets and Resources that are
 * part of your context. In general this context corresponds to a parent Application's context. If
 * you want to have your own service instance, you can use the {@link
 * TaskService#wrap(ScheduledExecutorService)} method to ensure that thread local variables are
 * correctly set.
 *
...
public class TaskService extends Service implements ScheduledExecutorService {
    ...
    private static class RestletThreadFactory implements ThreadFactory {
        ...
        /** Indicates whether the thread is a daemon thread. True by default. */
    ...
    /**
     * Wraps a JDK executor service to ensure that the threads executing the tasks will have the
     * thread local variables copied from the calling thread. This will ensure that call to static
     * methods like {@link Application#getCurrent()} still work.
     *
            ...
            public void execute(final Runnable runnable) {
                ...
                executorService.execute(
                        () -> {
                            ...
                            if (runnable instanceof ContextualRunnable contextualRunnable) {
                                ClassLoader tccl = Thread.currentThread().getContextClassLoader();
                                try {
                                    // Run the user task
                                    Thread.currentThread()
                                            .setContextClassLoader(
                                                    contextualRunnable.getContextClassLoader());
                                    runnable.run();
                                } finally {
    ...
    /** Indicates whether the threads are daemon threads. True by default. */
    private volatile boolean daemon;

    /**
     * Allow {@link #shutdown()} and {@link #shutdownNow()} methods to effectively shutdown the
    ...
    /**
     * Creates a new JDK executor service that will be wrapped. By default, it calls {@link
     * Executors#newCachedThreadPool(ThreadFactory)}, passing the result of {@link
     * #createThreadFactory()} as a parameter.
     *
     * @param corePoolSize The core pool size defining the maximum number of threads.
    ...
    /**
     * Creates a new thread factory that will properly name the Restlet created threads with a
     * "restlet-" prefix.
     *
     ...
     * all complete.<br>
     * <br>
     * Due to a breaking change between Java SE versions 5 and 6, and to maintain compatibility both
     * at the source and binary level, we have removed the generic information from this method. You
     * can check the {@link ExecutorService} interface for typing details.
     *
    ...
    /**
     * Executes the given tasks, returning a list of Futures holding their status and results when
     * all complete or the timeout expires, whichever happens first. Future.isDone() is true for
     * each element of the returned list. Upon return, tasks that have not completed are canceled.
     ...
     * not completed are canceled. The results of this method are undefined if the given collection
     * is modified while this operation is in progress.
     *
     * <p>Due to a breaking change between Java SE versions 5 and 6, and to maintain compatibility
     * both at the source and binary level, we have removed the generic information from this
     * method. You can check the {@link ExecutorService} interface for typing details.
     *
     * @param tasks The task to execute.
     ...
     * exceptional return, tasks that have not completed are canceled. The results of this method
    ...
    /**
     * Indicates if the {@link #shutdown()} and {@link #shutdownNow()} methods are allowed to
     * effectively shut down the wrapped executor service. Return false by default.
     *
     ...
     * @param delay The time from now to delay execution.
     * @param unit The time unit of the delay parameter.
     * @return a Future representing pending completion of the task, and whose get() method will
     ...
     * delay, and subsequently with the given period; that is executions will begin after
     * initialDelay then initialDelay+period, then initialDelay + 2 * period, and so on. If any
     * execution of the task encounters an exception, later executions are suppressed. Otherwise,
     ...
     * commencement of the next. If any execution of the task encounters an exception, later
     ...
     *
     * @return The list of tasks that never began execution;
     */
    public List<Runnable> shutdownNow() {
        return isShutdownAllowed() && (getWrapped() != null)
                ? getWrapped().shutdownNow()
        ...
        if (!isStarted()) {
            try {
                ...
                Context.getCurrentLogger()
                        .log(Level.WARNING, "Unable to start the task service", e);
            }
        }
    }
     ...
     * @return A Future representing pending completion of the task, and whose get() method will
     *     return the given result upon completion.
     */
    public <T> Future<T> submit(Callable<T> task) {
        startIfNeeded();
