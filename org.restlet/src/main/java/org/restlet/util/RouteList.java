...
/**
 * Modifiable list of routes with some helper methods. Note that this class implements the {@link
 * List} interface using the Route class as the generic type. This allows you to use an instance of
 * this class as any other {@link List}, in particular all the helper methods in {@link
 * Collections}.<br>
 * <br>
 * Note that structural changes to this list are thread-safe, using an underlying {@link
...
public final class RouteList extends WrapperList<Route> {
    ...
    public synchronized Route getNext(Request request, Response response, float requiredScore) {
        if (!isEmpty()) {
            for (final int initialIndex = this.lastIndex++;
    ...
    /**
     * Returns a random route match for a given call. Note that the current implementation doesn't
     * uniformly return routes unless they all score above the required score.
     *
     * @param request The request to score.
     * @param response The response to score.
     * @param requiredScore The minimum score required to have a match.
