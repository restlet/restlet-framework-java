...
/**
 * Representation able to apply an XSLT transformation. The internal JAXP transformer is created
 * when the getTransformer() method is first called. So, if you need to specify a custom URI
 * resolver, do it before actually using the representation for a transformation.<br>
 * <br>
 * This representation should be viewed as a wrapper representation that applies a transform sheet
 * on a source representation when it is read or written out. Therefore, it isn't intended to be
 * reused on different sources. For this use case, you should instead use the {@link
 * org.restlet.routing.Transformer} filter.
 *
 * @author Jerome Louvel
 */
public class TransformRepresentation extends WriterRepresentation {
    /**
     * Wraps a source representation into a {@link SAXSource}. This method can detect other {@link
     * XmlRepresentation} instances to use their {@link XmlRepresentation#getSaxSource()} method as
     * well as other {@link TransformRepresentation} instances to support transformation chaining.
     *
    ...
    public static SAXSource toSaxSource(Representation representation) throws IOException {
        ...
        switch (representation) {
            case XmlRepresentation xmlRepresentation -> result = xmlRepresentation.getSaxSource();
            case TransformRepresentation source -> {
                XMLReader reader =
                        new AbstractXmlReader() {

                            /**
                             * Parses the input source by sending the result event to the XML
                             * reader's content handler.
                             *
                             * @param input The input source.
                             */
                            public void parse(InputSource input) throws IOException, SAXException {
                                try {
                                    source.getTransformer()
                                            .transform(
                                                    source.getSaxSource(),
            ...
            default -> result = new SAXSource(new InputSource(representation.getReader()));
        }
     ...
     * @param source The source representation to transform.
     * @param transformSheet The XSLT transform sheet to apply.
     */
    public TransformRepresentation(
            Context context, Representation source, Representation transformSheet) {
        this((context == null) ? null : new ContextResolver(context), source, transformSheet);
    }
     ...
     * Constructor. Note that a default URI resolver will be created based on the given context.
     *
     * @param uriResolver The JAXP URI resolver.
     * @param source The source representation to transform.
     * @param transformSheet The XSLT transform sheet to apply.
     */
    public TransformRepresentation(
            URIResolver uriResolver, Representation source, Representation transformSheet) {
     ...
     * Constructor.
     *
     * @param uriResolver The optional JAXP URI resolver.
     * @param source The source representation to transform.
     * @param templates The precompiled JAXP template.
     */
    private TransformRepresentation(
     ...
     */
    public TransformRepresentation(
            URIResolver uriResolver, Representation source, Templates templates) {
        this(uriResolver, source, null, templates);
    }

    /**
     * Returns the transformer's error listener. The default value is null, leaving the original
     * listener intact.
     *
    ...
    /**
     * Returns the SAX source associated with the source representation.
     *
     * @return The SAX source associated with the source representation.
     * @throws IOException
     */
    ...
    /**
     * Returns the templates to be used and reused. If no one exists, it creates a new one based on
     * the transformSheet representation and on the URI resolver.
     *
    ...
    public Templates getTemplates() throws IOException {
        if (this.templates == null && getTransformSheet() != null) {
            try {
                ...
                final StreamSource transformSource =
                        new StreamSource(getTransformSheet().getStream());

                if (getTransformSheet().getLocationRef() != null) {
                    transformSource.setSystemId(
                            getTransformSheet().getLocationRef().getTargetRef().toString());
                }
    ...
    /**
     * Returns a new transformer to be used. Creation is based on the {@link
     * #getTemplates()}.newTransformer() method.
     *
    ...
    public Transformer getTransformer() throws IOException {
        ...
        try {
            Templates currentTemplates = getTemplates();

            if (currentTemplates != null) {
                result = currentTemplates.newTransformer();
            ...
            throw new IOException(
                    "Transformer factory configuration exception. " + tfce.getMessage());
        }
    ...
    /**
     * Returns the SAX transformer handler associated with the transform sheet.
     *
    ...
    public TransformerHandler getTransformerHandler() throws IOException {
        ...
        if (currentTemplates != null) {
            try {
                result = getSaxTransformerFactory().newTransformerHandler(currentTemplates);
            } catch (TransformerConfigurationException tce) {
                throw new IOException("Transformer configuration exception. " + tce.getMessage());
            }
        }
    ...
    public XMLFilter getXmlFilter() throws IOException {
        ...
        final Templates currentTemplates = getTemplates();

        if (currentTemplates != null) {
            try {
                result = getSaxTransformerFactory().newXMLFilter(currentTemplates);
            } catch (TransformerConfigurationException tce) {
                throw new IOException("Transformer configuration exception. " + tce.getMessage());
            }
        }
    ...
    /**
     * Releases the source and transform sheet representations, the transformer, and the URI
     * resolver.
     */
    @Override
    public void release() {
     ...
     *
     * @param outputProperties The JAXP transformer output properties.
     */
     ...
     *
     * @param parameters The JAXP transformer parameters.
     */
     ...
     *
     * @param templates The templates to be used and reused.
     */
     ...
     * @param result The JAXP result object.
     * @throws IOException
     */
    public void transform(Source source, Result result) throws IOException {
        if (getTransformer() == null) {
            Context.getCurrentLogger()
                    .warning("Unable to apply the transformation. No transformer found!");
        } else {
            try {
    ...
    /**
     * Writes the transformed source into the given JAXP result. The source is retrieved using the
     * {@link #getSaxSource()} method.
     *
     * @param result The JAXP result object.
     * @throws IOException
     */
    ...
    /**
     * Writes the transformed source into the given output stream. By default, it leverages the
     * {@link #write(Result)} method using a {@link StreamResult} object.
     */
    @Override
